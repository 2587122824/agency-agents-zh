...
| `PlanningRepository` | Creative Brief、Shot Plan 候选修订链及原子状态切换、PlanVersion、Shot、已确认实体引用和规划历史 | 规划中心服务 |
...
- 规划聚合的候选状态过滤、修订历史倒序、精确候选读取、`status + row_version` 原子切换、方案版本递增、镜头顺序和活动实体引用。
...
Repository 边界本身不包含 Provider 调用、重试、兜底、路由替换、提示词改写、状态转移或费用事件。Sprint 28 的候选修订功能新增独立 Alembic 迁移，但没有改变 Repository 的事务责任。
