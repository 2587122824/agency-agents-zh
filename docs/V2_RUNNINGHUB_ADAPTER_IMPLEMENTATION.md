...
- 设置页还会独立检查已发布工作流是否符合当前严格合同；历史不兼容配置优先显示“生成配置需要更新”，而不是只显示执行开关状态。
...
配置校验和只读连接准备共用 `providers/runninghub_contract.py`，避免历史已发布版本因旧验证状态而被误报为可执行。设置页的输入来源改为普通名称选择，固定值由界面编码为 `literal:<JSON>`；旧来源不会自动迁移。
