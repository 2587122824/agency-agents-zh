...
分镜候选的人工修订界面采用单镜头工作区：镜头导航显示编号、动作摘要、镜头类型、时长和修改状态，支持搜索以及“全部 / 已修改”筛选；编辑区只渲染当前镜头，提供上一个、下一个和重置当前镜头。桌面端左右两栏保持同一固定高度，镜头列表与右侧表单分别独立滚动，镜头数量不得撑高工作区或被底部操作栏截断。镜头列表提供互斥的“批量人物”和“AI 调整”选择模式，复选框随当前模式显示明确用途，点击选择框不切换当前编辑镜头。批量人物允许将一个明确人物版本追加到多个选中镜头，并可同时设为必须看清的人脸主体、主参考图和身份一致性要求；身份一致性与主参考图在界面中显式联动，没有已验证图片附件时不得设置主参考图。该操作只修改前端内存草稿并保留镜头已有的其他人物，最后仍由原合同计算逐镜头 Patch 并一次创建下一版候选。页面不自动保存、不调用分镜导演，也不改变旧候选。
...
“上传人物参考图”和“把人物用于镜头”是两个独立确认动作。前者创建已验证 Attachment、`identity_reference` 绑定和人物 EntityVersion；后者必须从当前正式 PlanVersion 显式开启 `revision_draft`，由用户逐镜头设置，或显式勾选多个镜头后批量应用同一人物版本、主参考图、`face_visibility`、人脸主体和身份一致性要求。批量应用只作用于选中镜头并保留原有人物，不代表自动应用全部镜头。
