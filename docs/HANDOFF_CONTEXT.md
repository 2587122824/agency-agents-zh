...
- Alembic runtime/head：`20260726_37`
...
旁白字幕、版本化音色/语速/音量、时长门禁、时间线交互、BGM/ducking/母带执行、确定性音频 QC 与人工试听确认，以及授权声音复刻、TTS/字幕精确重试和依赖级批量重试首期闭环均已完成。安全的 CosyVoice 真实验收工具已就绪，但当前配置仍缺少 DashScope API Key，真实付费合成和供应商试听未执行；这是七项范围当前唯一尚未完成的验收。配置凭据后必须由用户显式授权一次付费调用并保存真实请求 ID、非敏感用量与 WAV 探测证据。不得把执行参数、工具、授权登记或选择界面已就绪声明成真实供应商已经验收。QC 图片模型继续按用户要求暂不接入。
