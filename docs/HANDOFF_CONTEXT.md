...
- 新版剪辑台第四阶段进行中：`editor-preview.v1` 从已保存且合同哈希匹配的 Timeline 生成无交付副作用的本机 `draft_360p` 缓存。服务先重跑时间线和本机渲染合同校验，再复验全部本地输入文件哈希；合法版本按项目画幅使用长边 640、最高 24fps、libx264/veryfast/CRF30 渲染，缓存键冻结时间线合同、质量配置和 FFmpeg 版本，缓存读取继续复验 MP4 格式、尺寸和时长。`editor-preview-qc.v1` 已接入新生成和缓存结果：冻结 FFmpeg 检测持续黑画面；启用音轨时实测 EBU R128 响度、true peak 和音轨结束时间；字幕可读性、视觉连续性和主观音画同步明确保留人工复核。技术硬失败阻断放行但不剥夺预览播放，黑画面只作为警告。预览不确认时间线、不创建 Asset、DeliveryAttempt、交付 WorkItem 或费用。
...
- 剪映式第四阶段低清预览复验：API 与质量报告聚焦测试 `6 passed`，另有本机渲染器 `4 passed`；覆盖显式空位阻断、合法时间线 360×640 生成、缓存复用、内容读取、损坏缓存复验、无 DeliveryAttempt/交付 WorkItem/状态副作用，以及黑画面解析、响度/true peak 超限、分析器失败、音画时长偏差、字幕与主观人工复核。真实咖啡项目 v4 仍因 `TIMELINE_GAP_UNRESOLVED` 在 FFmpeg 前阻断并可定位 0.9 秒缺口；质量报告界面的合法预览路径仍需用无缺口真实时间线完成浏览器复验
