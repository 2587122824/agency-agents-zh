...
- 新版剪辑台第三阶段第一批已接入声音与字幕轨：已批准音频/字幕可拖入对应轨道，音频条目读取后端真实 PCM 波形并可试听，字幕条目读取真实 SRT；属性面板支持旁白/BGM、片段音量、BGM 权利依据/证据/确认和按相交旁白区间生成 ducking。声音/字幕条目可选择和移除，保存时按实际轨道冻结 enabled 状态。当前真实咖啡项目没有已批准音频或字幕，不得把空态验收表述为真实试听或拖放已通过。
...
- 剪映式第三阶段第一批复验：前端生产构建通过；真实 PCM 波形确定性缓存与 BGM 权利/循环/ducking/母带合同聚焦测试 `2 passed`。咖啡项目 v4 的声音轨显示“拖入已批准配音或 BGM”、字幕轨显示“拖入已批准字幕”，1280×720 无页面溢出且控制台无错误；因该项目没有已批准音频或字幕，真实拖放、试听、SRT 预览和新版本保存仍待合适真实项目验收
