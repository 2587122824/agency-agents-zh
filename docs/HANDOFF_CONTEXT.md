...
- 新版剪辑台高保真交互原型位于 `/editor-prototype`，现有剪辑台提供携带精确项目参数的入口。原型只读加载真实 EditorWorkspace、时间线和素材，所有调整仅在浏览器本地演示，不调用保存、校验、确认、生产或交付命令。首屏固定素材箱、完整比例监看窗口、右侧片段/缺口任务和底部三轨时间线；真实空位提供缩短成片、生成补充镜头、选择其他已批准素材三条合法路径。
