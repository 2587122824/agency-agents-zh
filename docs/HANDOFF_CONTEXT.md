...
V2 正在独立于 V1 建设合同驱动、状态可审计的 AI 视频生产系统。六个智能体与确定性生产编译器的设计已经确认；创作制片人、内容策划、分镜导演、制作规划、质量审核首期图片合同和剪辑助理已经实施，剪辑候选审核、本机 FFmpeg 合成和最终交付验证闭环也已实现。QC 图片模型仍按用户要求暂不接入：
...
| 已发布配置 | `production_config_86e67cba7404453eaba2340f8c60b0a5`，版本 53；仅将人物一致性关键帧 `2073414172825706497` 的 `24.denoise` 从误迁移的 `0.32` 修正为 V1 真实成功请求使用的 `1.0`，其他工作流、模型、NodeInfoList、供应商和媒体策略与 v52 相同 |
...
- 当前生产配置：v53，5 个文本智能体模型分工与 6 个镜头工作流槽位；人物一致性关键帧 `2073414172825706497` 的 `24.denoise=1.0` 来自 V1 真实成功请求证据；供应商 API Key 使用配置普通字段；本地素材库引用为 `v2.runtime.assets`；`2072296894507872257` 为首中尾帧生视频，绑定 `447 / 448 / 449` 三张独立父图；剪辑助理合同为 `editor-assistant-input.v1 / timeline-candidate.v1 / editor-assistant-prompt.v1`
...
创作制片人、内容策划、分镜导演、制作规划、剪辑助理和本机 FFmpeg 最终交付已完成。质量审核智能体的图片合同、候选持久化、运行审计、失败精确重跑和人工确认边界已完成，但 DeepSeek 官方 API 当前没有文档化图片输入，不能作为 QC 图片 Provider；按用户要求暂不接入 QC 模型，视频与音频继续保持显式人工审核。单个生产工作项的用户确认精确重跑已实现；依赖级批量重试、音频接入或其他新能力仍需单独确认，不得自行扩展。
