...
提交 `69bbc8ee` 的进程身份重启门禁已于 2026-07-30 使用管理员 PowerShell 复验；提交 `7674c78e` 发布逐 cue 字幕修订后，API PID 已从 `58324` 切换为 `26216`，Worker PID 已从 `51736` 切换为 `30080`。API 日志记录新进程启动，健康检查为 `ok`，Alembic runtime/head 均为 `20260730_41`。修复后的脚本已同时证明旧实例无法结束时会失败、成功时只接受本次新 API PID 的健康响应。
...
- 新版剪辑台四阶段功能和真实整链验收均已完成：`editor-preview.v1` 从已保存且合同哈希匹配的 Timeline 生成无交付副作用的本机 `draft_360p` 缓存。`editor-preview-qc.v1` 检测持续黑画面，并在启用音轨时实测 EBU R128 响度、true peak 和音轨结束时间；字幕可读性、视觉连续性和主观音画同步保留人工复核。新版弹窗提供逐项清单及源时间线/渲染结果同步双窗，结果播放头跨片段时自动切换源素材并映射源时点。提交时重新复验 Timeline 行版本/合同哈希、缓存键、预览文件哈希和质量报告，成功新增不可变 `timeline.preview_reviewed.v1` 并投影为 `editor-preview-review.v1`；Timeline 读取会按精确合同恢复已有复核。用户随后可在同一剪辑台显式确认时间线，再选择 `local_ffmpeg / external_upload` 并授权；授权后面板支持本机任务状态、外部 MP4 上传、输出显式验证、阻断证据与已验证成片下载。各命令保持独立，不自动授权、重试、切换或验证；预览与复核本身均不确认时间线、不产生费用。
...
- Alembic runtime/head：`20260730_41`
