...
V2 正在独立于 V1 建设合同驱动、状态可审计的 AI 视频生产系统。六个智能体与确定性生产编译器的设计已经确认；创作制片人、内容策划、分镜导演、制作规划、质量审核首期图片合同和剪辑助理已经实施，剪辑候选审核、CosyVoice 旁白合同、本机 FFmpeg 音视频合成和最终交付验证闭环也已实现。CosyVoice 当前缺少配置 API Key，尚未做真实付费验收；QC 图片模型仍按用户要求暂不接入：
...
- 迁移 `20260726_38` 新增不可变 CosyVoice 真实验收记录：系统配置页只读预检冻结配置哈希、默认音色/语速/音量、文本哈希与 24kHz 单声道 WAV 合同；凭据和外部执行授权就绪后仍须二次勾选才执行一次真实调用。已授权尝试保存是否联网、请求 ID、非敏感用量、WAV 探测或明确失败证据，同命令保持幂等且任何响应不含 API Key。当前 v55 缺少 Key，按钮禁用且未创建验收尝试。
...
- 系统配置页 CosyVoice 真实验收入口：只读预检显示 v55、`cosyvoice-v1`、`longxiaochun / rate=1.0 / volume=50` 与 24kHz 单声道 WAV 合同；当前缺 Key 时付费按钮禁用、记录数为 0、`network_probe=false`；390×844 下 `scrollWidth == clientWidth`
...
旁白字幕、版本化音色/语速/音量、时长门禁、时间线交互、BGM/ducking/母带执行、确定性音频 QC 与人工试听确认，以及授权声音复刻、TTS/字幕精确重试和依赖级批量重试首期闭环均已完成。CosyVoice 真实验收已具备命令、只读 API、设置页二次确认和不可变证据记录，但当前配置仍缺少 DashScope API Key，真实付费合成和供应商试听未执行；这是七项范围当前唯一尚未完成的验收。配置凭据后必须由用户显式授权一次付费调用，只有持久化 `passed` 记录包含真实请求 ID、非敏感用量与 WAV 探测证据时才能完成。不得把执行参数、工具、授权登记或选择界面已就绪声明成真实供应商已经验收。QC 图片模型继续按用户要求暂不接入。
