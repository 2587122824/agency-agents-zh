...
- 新版剪辑台第四阶段进行中：`editor-preview.v1` 从已保存且合同哈希匹配的 Timeline 生成无交付副作用的本机 `draft_360p` 缓存。`editor-preview-qc.v1` 对新生成和缓存结果检测持续黑画面，并在启用音轨时实测 EBU R128 响度、true peak 和音轨结束时间；字幕可读性、视觉连续性和主观音画同步保留人工复核。新版弹窗提供逐项观看清单及源时间线/渲染结果同步双窗，结果播放头跨片段时自动切换源素材并映射源时点。提交时重新复验 Timeline 行版本/合同哈希、缓存键、预览文件哈希和质量报告，成功新增不可变 `timeline.preview_reviewed.v1` 并投影为 `editor-preview-review.v1`；Timeline 读取会按精确合同恢复已有复核。用户随后可在同一剪辑台显式确认时间线，再选择 `local_ffmpeg / external_upload` 并授权，两个命令仍保持独立。技术阻断不能提交复核，黑画面只告警；预览与复核本身均不确认时间线、不产生费用。下一步把外部上传、本机渲染进度、输出验证和成片下载接入新版剪辑台。
