...
- 剪辑时间线使用 `v2.timeline-contract.v2`：冻结并持久化缩放与吸附设置，支持同轨拖放重排、源区间/成片区间一致裁切、真实 PCM WAV 波形缓存、视频淡入淡出转场以及音频关键点音量包络。本机 FFmpeg 确定性执行相应滤镜，交付 Manifest 复验冻结合同；既有开发时间线由迁移 `20260726_35` 显式回填，不在运行时代码中兼容旧合同。
...
- Alembic runtime/head：`20260726_35`
...
旁白字幕、版本化音色/语速/音量、时长门禁，以及波形、裁切、同轨拖放、缩放/吸附、淡入淡出转场和音量包络的首期闭环均已完成。安全的 CosyVoice 真实验收工具已就绪，但当前配置仍缺少 DashScope API Key，真实付费合成和供应商试听未执行。完整七项后续范围以 [V2 配音与剪辑完善路线图](./V2_AUDIO_EDITING_ROADMAP.md) 为权威：下一步实施 BGM 资产授权与选择、循环/裁切、旁白区间 ducking、响度目标、true-peak/削波门禁及确定性 FFmpeg 执行；之后补齐完整确定性音频 QC、授权声音复刻和依赖级费用确认批量重试。不得把工具或选择界面已就绪声明成真实供应商已经验收。QC 图片模型继续按用户要求暂不接入。
