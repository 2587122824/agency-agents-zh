...
| 日期 | 变更 |
|---|---|
| 2026-07-23 | 新增单个生产步骤精确重跑：命令绑定活动快照合同哈希、当前失败尝试和原请求指纹，用户确认新增费用后只创建一个新 WorkAttempt 与对应费用事件；生产页提供逐项确认入口。旧失败不覆盖，不修改提示词/Provider/工作流/输入，不连带重跑其他任务；提交结果未知仍要求人工对账 |
