...
- 剪辑时间线的 v2 交互基础冻结并持久化缩放、磁吸开关与吸附间隔，支持同轨拖放重排、源区间/成片区间一致裁切、真实 PCM WAV 波形缓存、视频淡入淡出转场以及音频关键点音量包络。本机 FFmpeg 确定性执行相应滤镜，交付 Manifest 复验冻结合同；既有开发时间线由迁移 `20260726_35` 与 `20260729_40` 显式回填，不在运行时代码中兼容旧合同。
...
- 剪辑时间线交互验收：`v2.timeline-contract.v3` 持久化 `pixels_per_second=20..400`、`snap_enabled` 与 `snap_interval_ms=10..1000`；同轨拖放确定性重排并重算时间，源区间与成片区间裁切保持时长一致；PCM WAV 波形按内容哈希和 bin 数缓存；视频转场仅接受 `cut/fade`，音频包络要求 2–64 个严格递增且覆盖片段边界的 `-60dB..12dB` 关键点；FFmpeg 渲染与交付 Manifest 均按冻结合同执行或阻断
