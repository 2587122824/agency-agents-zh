...
| 日期 | 变更 |
|---|---|
| 2026-07-28 | 真实首中尾三帧项目完成 9 张图片调用后确认连续性不达标：三帧原先各自用单帧状态覆盖共同提示词，且 seed 按 start/middle/end 节点分裂。编译器现为同镜头三帧注入共同视觉与连续性锁定、共享确定性 seed，只保留逐帧状态差异；图片人工拒绝同步补齐 `execution_blocked -> 用户确认结束 -> contract_ready` 闭环，取消未开始任务并保留历史素材和费用证据。完整后端基线 `294 passed`、前端生产构建通过；未批准旧图片、未放行视频、未自动追加付费重跑 |
