...
- The `内容栏目` controls were moved out of the hidden legacy task-info section into the visible main composer. Running a task now automatically injects the selected column constraints even if the user does not click `套用栏目模板`. Generated asset cards now show a dedicated high-z-index favorite button, mark already favorited source assets as `已收藏`, avoid duplicate library copies, and the asset lightbox closes from the close button, Esc, or any non-media/non-button blank area.
