...
- 时间线音频合同升级为 `v2.timeline-contract.v3`：BGM 必须冻结用户确认的自有/许可/免版税权利依据与证据，可显式裁切或循环；ducking 区间必须与当前旁白片段精确一致，FFmpeg 按冻结 reduction/attack/release 执行。迁移 `20260726_36` 显式回填 LUFS、dBTP 与 limiter 母带合同；真实本机循环、ducking、`loudnorm + alimiter` 和 H.264/AAC 合成已通过。执行证据不冒充输出实测 QC，响度、true peak、削波与静音测量属于下一步。
...
旁白字幕、版本化音色/语速/音量、时长门禁、时间线交互，以及 BGM 权利确认、循环/裁切、旁白区间 ducking、响度目标和 limiter 执行首期闭环均已完成。安全的 CosyVoice 真实验收工具已就绪，但当前配置仍缺少 DashScope API Key，真实付费合成和供应商试听未执行。完整七项后续范围以 [V2 配音与剪辑完善路线图](./V2_AUDIO_EDITING_ROADMAP.md) 为权威：下一步增加采样率、声道、静音比例、连续静音、响度、true peak、削波与目标时长的确定性音频 QC，并在规则通过后要求用户试听和显式确认；之后实施授权声音复刻和依赖级费用确认批量重试。不得把执行参数、工具或选择界面已就绪声明成输出 QC 或真实供应商已经验收。QC 图片模型继续按用户要求暂不接入。
