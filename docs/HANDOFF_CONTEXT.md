...
- `04_keyframe/img2img_style_keyframe` is a separate stabilization-phase submode for reference-image-based keyframes that should preserve source subject/composition. Its active nodeInfoList is calibrated to the user-provided Flux/Kontext img2img workflow `风格参考图生图.json`, mapping `LoadImage(81)`, `CLIPTextEncode(39/40)`, resize primitives `90/89`, `KSampler //Inspire(86)`, and `SaveImage(50)`. Width, height, seed, and denoise nodeInfo values must use form placeholders (`{{width}}`, `{{height}}`, `{{seed}}`, `{{denoise}}`) rather than fixed workflow defaults. Default runtime `denoise=0.45` remains a conservative starting point; lower values preserve more of the source image.
