...
创作制片人、内容策划、分镜导演、制作规划、剪辑助理和本机 FFmpeg 最终交付已完成。质量审核智能体的图片合同、候选持久化、运行审计、失败精确重跑和人工确认边界已完成，但 DeepSeek 官方 API 当前没有文档化图片输入，不能作为 QC 图片 Provider；按用户要求暂不接入 QC 模型，视频与音频继续保持显式人工审核。下一阶段应由用户先验收剪辑确认、本机生成、交付验证和下载闭环；精确重试、阻断恢复、音频接入或其他新能力仍需单独确认，不得自行扩展。
