...
- LTX2.3 first-frame image-to-video (`06_i2v_first_frame / i2v_first_frame`) uses the user's current canvas from `lix2.3图生视频（首帧）.json`. Its nodeInfo must map form values to: positive `4923.prompt` and `2483.text`, negative `4961.prompt` and `2612.text`, first frame `2004.image`, size/frame count `3059.width/height/length`, shared frame count `4979.value` and `3980.frames_number`, FPS `4978.value`, `3980.frame_rate`, `1241.frame_rate`, `4849.fps`, `4819.fps`, seed `4967.seed`, and output prefixes `4852/4823.filename_prefix`. Keep `3159.strength=1` and bypass disabled through `3159.bypass=false` plus `4977.value=false` so uploaded first-frame images actually drive the output.
