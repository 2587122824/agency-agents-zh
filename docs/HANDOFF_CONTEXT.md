...
- 素材审核页已改为连续工作台：顶部项目与进度、左侧当前快照分镜队列和显式多选、中间真实比例大图/缩放/全屏/前后切换、右侧冻结分镜与文件事实。`verified` 素材无需不可用的 QC 模型即可直接人工通过并形成 `human-review.v1` 报告；“需要调整”先选处理范围再点选结构化原因，补充说明默认可选且仅“其他”必填，操作后进入对应工作区。批量通过只处理确认框列出的选择，`waiting_phase / queued / running` 节点不再误报输出缺口。
