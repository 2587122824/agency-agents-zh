...
## Remove Separate Media Prompt Inputs

[2026-06-15 +08:00] command: removed the separate user-facing image/video prompt inputs after the user decided that media prompts should be generated by the workflow staff instead of manually filled in the management UI.

Product decision:

- The user should provide the original demand and optional reference images.
- `02_短视频编导` owns content direction, creative structure, visual language, and shot rhythm.
- `06_分镜生图设计师` turns the director output into keyframe/image prompt plans.
- `07_视频生成执行员` turns storyboard and reference-image guidance into video-generation packages.
- `21_ComfyUI成片编排师` maps generated prompts, reference images, voice, subtitles, and final-production requirements into ComfyUI/RunningHub node parameters.

Management UI behavior:

- The `生图配置` and `视频生成配置` sections are hidden from the user.
- Hidden DOM controls remain only for JS/localStorage compatibility.
