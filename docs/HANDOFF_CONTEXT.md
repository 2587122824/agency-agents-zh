...
- 旁白模式把已确认 Creative Brief 中有序的 `voiceover/dialogue` 文本冻结进单一 TTS 节点；CosyVoice Adapter 只提交冻结模型、预设音色、格式和采样率，下载后交叉验证 WAV 签名、文件大小和哈希，再登记为待人工审核音频素材。剪辑助理只接受当前快照唯一已批准旁白并确定性加入音频轨；本机 FFmpeg 按源区间和时间线入点执行裁剪、延迟、混音及 AAC 编码。
...
创作制片人、内容策划、分镜导演、制作规划、剪辑助理和本机 FFmpeg 音视频最终交付已完成。CosyVoice 预设音色旁白合同和 Adapter 已接入，但必须先在当前配置填写 DashScope API Key 才能执行真实合成；声音复刻、BGM、字幕烧录和临时 OSS 音频不在当前范围。质量审核智能体的图片合同、候选持久化、运行审计、失败精确重跑和人工确认边界已完成，但 DeepSeek 官方 API 当前没有文档化图片输入，不能作为 QC 图片 Provider；按用户要求暂不接入 QC 模型，视频与音频继续保持显式人工审核。单个生产工作项的用户确认精确重跑已实现；依赖级批量重试或其他新能力仍需单独确认，不得自行扩展。
