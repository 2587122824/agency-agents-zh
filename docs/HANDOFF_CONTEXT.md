...
- `04_keyframe/identity_scene_keyframe` is a stabilization-phase submode for shots that must keep both a linked character and a linked scene. It requires `input_identity_image` plus `input_scene_image`, uses `control_mode=identity_scene_reference`, and should be selected by production only when both references are resolved. The current default nodeInfo maps the character to the identity LoadImage slot and the scene reference to the secondary LoadImage slot of the identity workflow; it is intentionally isolated so a later calibrated dual-reference scene workflow can replace the nodeInfo without renaming existing modes.
