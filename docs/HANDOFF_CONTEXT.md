...
- 新版剪辑台第四阶段已开始：`editor-preview.v1` 从已保存且合同哈希匹配的 Timeline 生成无交付副作用的本机 `draft_360p` 缓存。服务先重跑时间线和本机渲染合同校验，再复验全部本地输入文件哈希；合法版本按项目画幅使用长边 640、最高 24fps、libx264/veryfast/CRF30 渲染，缓存键冻结时间线合同、质量配置和 FFmpeg 版本，缓存读取继续复验 MP4 格式、尺寸和时长。阻断返回结构化问题，不启动 FFmpeg；预览不确认时间线、不创建 Asset、DeliveryAttempt、交付 WorkItem 或费用。
...
- 剪映式第四阶段低清预览复验：API 聚焦测试 `2 passed`，覆盖显式空位阻断，以及合法时间线 360×640 生成、缓存复用、内容读取、损坏缓存复验和无 DeliveryAttempt/交付 WorkItem/状态副作用。真实咖啡项目 v4 点击“低清预览”后没有启动 FFmpeg，弹窗显示 `TIMELINE_GAP_UNRESOLVED`，点击后定位 0.9 秒缺口；页面明确不确认、不交付、不产生供应商费用，1280×720 无溢出且控制台无错误
