...
- 新版剪辑台第四阶段进行中：`editor-preview.v1` 从已保存且合同哈希匹配的 Timeline 生成无交付副作用的本机 `draft_360p` 缓存。`editor-preview-qc.v1` 对新生成和缓存结果检测持续黑画面，并在启用音轨时实测 EBU R128 响度、true peak 和音轨结束时间；字幕可读性、视觉连续性和主观音画同步保留人工复核。新版弹窗提供逐项观看清单；提交时重新复验 Timeline 行版本/合同哈希、缓存键、预览文件哈希和质量报告，成功新增不可变 `timeline.preview_reviewed.v1` 并投影为 `editor-preview-review.v1`。技术阻断不能提交复核，黑画面只告警。正式交付升级为 `v2.delivery-request.v3`，必须冻结同合同复核证据；缺少复核时交付页导航回新版剪辑台，不创建 Attempt/WorkItem。预览与复核本身均不确认时间线、不产生费用。
...
- 剪映式第四阶段预览/复核/交付门禁复验：聚焦 API `11 passed`、本机渲染器 `4 passed`、完整 API `123 passed`、前端构建通过；覆盖显式空位阻断、合法预览生成/缓存/内容读取/损坏复验、黑画面与音频检查、人工勾选缺失、复核幂等、无复核禁止授权、复核证据冻结进 `v2.delivery-request.v3` 及执行/验证重建一致性。完整 API 首轮暴露真实开发环境外部执行开关污染四项“默认关闭”测试；fixture 已显式隔离为 false，需要执行的专项测试自行开启，修复后完整通过且不改变运行服务配置。真实咖啡项目 v4 仍因 `TIMELINE_GAP_UNRESOLVED` 在 FFmpeg 前阻断并可定位 0.9 秒缺口；合法预览复核界面仍需用无缺口真实时间线完成浏览器复验
