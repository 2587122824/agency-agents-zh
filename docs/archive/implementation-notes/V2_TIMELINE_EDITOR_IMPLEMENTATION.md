...
候选列表新增 exact-key“已完整对照 A→B”观看记忆。只有现有两阶段状态机在 tuned 阶段真实抵达右侧终点、证据仍匹配时才登记当前合法单侧邻帧候选；等待证据、停止、只看 A 或证据失效均不登记。切换候选保留已完成标记，基础条目、素材、源窗、fps 或边界变化时与实测动作缓存一起清空。真实页面中 `+2帧` 开始等待后 100ms 清除，5 秒后仍为 0 个完成标记；完整观看 `+1帧`、保留 A、再完整观看 `+2帧` 后，两张卡同时保留标记。两个 203×54 标记和 232px 扫描区无自身溢出，1280×720 页面无横向溢出，页面日志为空；草稿保持 `row_version=230 / playhead_ms=0 / SH-001 source=0..409ms`。该标记只帮助用户区分测过与看完，不表示评价、推荐、采用或正式复核，也不进入草稿、撤销历史、API 或持久存储。
