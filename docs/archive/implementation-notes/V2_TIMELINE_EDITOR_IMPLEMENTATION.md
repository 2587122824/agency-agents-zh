...
- 倒序边界现在先投影紧邻 gap 的完整确定性修复。命中时摘要展示延长前镜、唯一正式 Shot、唯一 filler 与补齐目标，主入口只导航到 gap；“仅交换这两镜”降为次级。Gap Inspector 的完整修复保持一级，其他拆分路径进入原生 details；实际完整事务仍只由 `applySelectedGapCombinedRepair(true)` 执行，没有新增编辑分支。
