...
- 发布 `2cd51ab4`：完整后端 `306 passed in 188.00s`，compileall、Vite `/assets/index-BLOXXDyT.js`、diff check 通过；真实 v12 将 SH-002→SH-001 局部交换为 SH-001→SH-002，自动试听并登记 1 个新切点完整上下文，一次撤销恢复倒序并再次试听。1280×720 为 `1280/1280`、Inspector `244/244`、页面日志为空；清理后 draft null、播放头 0。最终 API `27456` / Worker `56584`、Alembic `20260813_51`、健康 `ok`、标准四日志零实际错误。
- Inspector 顶层改为互斥任务视图：默认片段态只保留片段身份、范围、操作和一张衔接摘要；点击“检查当前切点”或消费连续性/候选待办后才挂载完整衔接工作区，标题同步改为“衔接检查”，并提供“返回片段”。`boundaryInspectorOpen` 仅为页面态；切换不设置 B、不播放、不写草稿、不产生 history，不改变现有边界审核合同。
- 发布 `a69731d4 / 916e9a62`：完整后端隔离外部执行后 `306 passed in 180.64s`，Python compileall、Vite `/assets/index-CKyszVTd.js`、diff check 通过。真实咖啡 v12 默认态包含“片段属性 / 3 项待检查 / 检查当前切点”，不挂载滚动剪辑或转场；进入衔接态包含“衔接检查 / 返回片段 / 滚动剪辑 / 转场”，且不再挂载片段操作；返回后恢复默认层级。1280×720 的 html/body `1280/1280`、Inspector `244/244`、页面 warning/error 为空；最终刷新恢复 Timeline v12、播放头 0、draft null。标准服务 API `20512` / Worker `57248`，创建时间均为 `2026-08-16 09:46:18`；健康 `ok`、Alembic runtime/head `20260813_51`，标准四日志零实际错误。
- 顶部移除状态条的“查看检查问题 / 处理衔接检查”并列入口，主操作统一显示结构与连续性汇总待办。普通 Inspector 同步派生当前边界正式序号；明确倒序时用局部交换替代泛化检查作为主操作，并把衔接工具保留为次级文字入口。该路径直接复用既有 `swapBoundaryToFormalOrder`、锁轨门禁、一次 history 与 `structure` 自动试听，不新增持久状态。
- 发布 `52007de0`：完整后端隔离外部执行后 `306 passed in 182.24s`，Python compileall、Vite `/assets/index-D8-K8Tdg.js`、diff check 通过。真实咖啡 v12 顶部唯一主操作显示“处理 4 项待办”，旧“查看 1 个检查问题 / 处理 3 个衔接检查”均不存在；普通倒序摘要显示结构原因、局部交换主操作和次级衔接入口，锁轨时交换按钮可见但 disabled，次级入口进入/返回后 undo 仍禁用。交换后只形成 `SH-001 0.4s → SH-002 4.7s → gap 9.9s`，倒序警告消失，自动试听 1 个新切点并登记 1× 完整上下文；一次撤销恢复 `SH-002 4.7s → SH-001 0.4s → gap 9.9s`、倒序入口和 1 个恢复切点复检。验收草稿已丢弃，最终 Timeline v12、播放头 0、draft null；1280×720 html/body `1280/1280`、Inspector `244/244`、页面 warning/error 为空。标准服务 API `57112` / Worker `51164`，创建时间均为 `2026-08-16 09:55:32`；健康 `ok`、Alembic runtime/head `20260813_51`，标准四日志零实际错误。
- 顺序告警按问题规模分层：单处倒序的时间线按钮改为只读“定位这处倒序”，选择精确边界并把普通 Inspector 的局部交换摘要滚入视野；多处倒序才显示“整理全部倒序镜头”。单处定位不消费锁轨、不产生 history 或草稿，实际修改仍只有右侧局部交换。
- 发布 `7f6938ea`：完整后端隔离外部执行后 `306 passed in 182.29s`，Python compileall、Vite `/assets/index-DljkgQox.js`、diff check 通过。真实咖啡 v12 的单处告警只显示“定位这处倒序 / 在右侧局部修复”，旧全局整理文案不存在；锁轨时定位仍 enabled、局部交换 disabled、undo disabled，定位把摘要从 Inspector 可视区外 `top 481 / bottom 640.875` 滚到 `top 253 / bottom 412.875`。随后用正式 UI 补入 SH-003 并两次向前移动，构造 `SH-003 → SH-002 → SH-001` 两处倒序；此时只显示“整理全部倒序镜头”，单处定位消失，锁轨改为 disabled“解锁后整理全部”，执行后成为 `SH-001 → SH-002 → SH-003` 且告警消失。验收草稿已丢弃，最终 Timeline v12、原 `SH-002 → SH-001 → gap`、播放头 0、draft null；1280×720 html/body `1280/1280`、Inspector `244/244`、页面 warning/error 为空。关闭 Browser 后再次标准重启，最终 API `21388` / Worker `43836`，创建时间均为 `2026-08-16 10:10:23`；健康 `ok`、Alembic runtime/head `20260813_51`，标准四日志零实际错误。
- 倒序边界现在先投影紧邻 gap 的完整确定性修复。命中时摘要展示延长前镜、唯一正式 Shot、唯一 filler 与补齐目标，且只保留导航到 gap 的主入口；“仅交换这两镜”只在无法形成完整事务时回退出现。Gap Inspector 的完整修复保持一级，其他拆分路径进入原生 details；实际完整事务仍只由 `applySelectedGapCombinedRepair(true)` 执行，没有新增编辑分支。
- 复杂度收敛补充：真实 1280×720 验收发现完整修复与“仅交换这两镜”仍在普通片段态并列；已移除后者。完整修复可确定时默认层现在只有一个入口，局部交换仅作为无法形成完整事务时的回退。
- 顶部主操作从原子检查项总数改为阶段任务投影。倒序优先定位右侧确定性结构摘要，只有无倒序时才定位首个 gap；结构与其他验证问题清零后，连续性按未完成边界数量显示“复检 N 个切点”。这同时修复了旧点击顺序先进入连续性、后处理结构的语义倒置。
...
- 普通衔接复检进一步与修复工具分层。`repairToolsVisible` 只在当前问题上下文仍为 `needs_adjustment` 或存在候选续办时成立；默认不渲染切点预览参数、循环、滚动剪辑、双侧滑移、转场 A/B、模式条、外层转场编辑和手工连续巡检。观察入口仍直接挂载所需 mode 的唯一内容。检查行在无结论时移除冗余“未检查”，证据未就绪时不渲染“通过”，已有结论才显示“重新判断”。没有新增 schema、API、迁移或运行时兼容。
...
- 本轮按问题类型拆分修复路径：sequence `issue` 不再落入邻帧动作扫描，而是先给与当前成对转场相反的唯一轻量 B，再使用既有顺序舞台完成 A→B；sequence 的证据 ready 不依赖像素/动作探针，动作证据、候选和相位 DOM 均不挂载。action 继续沿用原候选、同步动作、证据和相位闭环。
- 外层工具改按当前 handling context 的 mode 投影：frames/overlay 保留直接预览、滚动、滑移、转场和四模式条，action/sequence 只显示内部专业路径，候选续办直接 action。真实咖啡 v12 在 1280×720 验收首个 `SH-001 → SH-002` 时间跳转：处理态只有 `试用 0.2s 淡出淡入` 与折叠的“其他节奏方案”，邻帧、同步动作、详细动作证据、外层滑移/转场均为 0；B 生成后立即可开始 A→B，完整播放后出现“保留 A / 采用 B”。基线倒序边界的 action 问题仍显示后镜 ±2 帧候选、同步播放和详细动作证据。html/body `1280/1280`，页面 warning/error 为空，验收草稿两次原子丢弃并恢复 Timeline v12、播放头 0、undo disabled、draft null。
- 发布证据：`bc8dd958` 已推送 `main`；完整后端 `307 passed in 207.59s`，compileall、Vite `index-DNK6wUKV.js / index-hozuMUdC.css`、diff check 通过。功能提交后标准重启 API `58172` / Worker `50596`，health `ok`，8766 listener 等于 API PID，Alembic runtime/head `20260816_52`，权威草稿响应 `null`，四份标准日志无实际错误。
- sequence 首选 B 从统一 `fade:200` 改为检查语义驱动。`BoundaryCandidateGuidanceRequest.checkId` 传入比较组件，`SEQUENCE_TRANSITION_RECOMMENDATIONS` 将时间跳转、地点变化、换装分别冻结为 `200 / 300 / 500ms` 及可读原因；片段过短时继续由现有合法最大值裁剪。基线只有精确命中当前推荐时才用 cut 作为唯一对照，其他 fade 时长改为推荐 fade，避免“只要已有淡出就退回硬切”的错误路径。界面仍只有一个主按钮和折叠的其他节奏方案，不新增合同、持久字段、迁移或模型判断。
- 真实咖啡 v12 在 1280×720 完整执行一次确定性四段修复后，首个 `SH-001 → SH-002` 时间跳转处理态只显示 `试用 0.2s 淡出淡入` 和“短黑场标出时间略过”原因；B 生成后立即可开始 A→B，完整对照显示保留/采用。采用 200ms 后再次标记同一问题并进入处理，基线精确命中分支只显示 `试用直接切换` 与相应原因，证明不是“任意 fade 均退回 cut”。正式项目三条 continuity relation 均为 time_jump；location/outfit 的 300/500ms 由静态推荐表、TypeScript 枚举与生产构建覆盖。验收草稿通过页面顶栏原子丢弃，恢复 Timeline v12、播放头 0、媒体播放数 0、draft null；html/body `1280/1280`，生产 bundle `index-lAxesmtH.js`，页面 warning/error 为空。完整后端隔离外部执行后 `307 passed in 209.95s`，Python compileall、Vite build 与 diff check 通过。
- 发布证据：`0f3b191f` 已推送 `main`。标准重启 API `56416` / Worker `55552`，创建时间 `2026-08-20 09:32:05`；health `ok`、8766 listener 等于 API PID、Alembic runtime/head `20260816_52`、draft null，API 日志仅健康/草稿 GET 与正常启动，Worker 日志为空。
- sequence 首选 B 现在使用稳定 `sequence-primary:<checkId>:<type>:<durationMs>` 替代审核键。完整 A→B 后选择保留 A，复用既有 `alternative_outcomes / kept_baseline` 持久化；同一 exact candidate session 再次进入时，主卡不重复生成该 B，而原位显示“推荐节奏已排除”与唯一“转到片段与素材处理”。该入口复用结构回退意图并补齐 `boundaryInspectorOpen=false`，真正回到简洁片段态；锁轨时禁用。普通高级方案仍在折叠区，边界 session 变化自然重新评估，不新增合同或迁移。
- Browser 跨刷新验收证明：咖啡 v12 首个时间跳转保留 `fade:200` B 后，服务端 candidate session 只新增 `sequence-primary:jump-readable:fade:200=kept_baseline`；刷新恢复草稿、回到该 boundary 并再次处理时，原试用按钮 0、升级入口 1。点击升级后页面显示片段属性、切点节奏 region 0，主画面四段时点与素材身份不变；仅片段选择/播放头通过既有草稿自动保存，不产生 timeline item 修改或 history。最终正式 UI 丢弃草稿，恢复 v12、播放头 0、draft null、html/body `1280/1280`、媒体 0、页面日志为空，bundle `index-bleN4LeU.js`。完整后端 `307 passed in 208.17s`，compileall、Vite build 与 diff check 通过。
- 发布证据：`918dbd33` 已推送 `main`。标准重启 API `56936` / Worker `57440`，创建时间 `2026-08-20 09:45:30`；health `ok`、8766 listener 等于 API PID、Alembic runtime/head `20260816_52`、draft null，API 日志仅健康/草稿 GET 与正常启动，Worker 日志为空。
- sequence 排除态的唯一续办改为素材事实驱动。父级统计当前未使用、已批准且有正时长的视频数：有候选时主卡只显示“为后镜选择替代素材”并复用 `startBoundaryAssetReplacement(right)`；无候选时只显示“调整镜头结构”。素材替换入口补齐关闭 boundary Inspector，直接进入简洁片段态和既有素材箱过滤，不自动替换。审计确认现有 production retry 只服务失败 WorkItem，故没有添加虚假的成功镜头重生成按钮，也未调用云端 Provider、扩展 API 或合同。
- 真实 Browser 验收在“替换 SH-002”模式发现草稿丢弃后的瞬时过滤泄漏：Timeline 已恢复 v12，但素材箱仍只显示替换候选。修复后 `discardDraft` 只在服务端 DELETE 成功路径原子清理素材替换、缺口选择、搜索、衔接 Inspector、候选/帧/roll/连续复检状态；失败路径保持零清理。该修复不改变草稿合同、Timeline 或后端 API。
- 修复后 Browser 复验从正式咖啡 v12 重建四段、排除首个 `fade:200`，再移除末尾补充素材形成唯一 `provider_output` 候选；进入“替换 SH-002”时只见该候选且片段 Inspector 保持简洁。直接丢弃后替换 banner 与切点节奏 region 均为 0、四素材恢复、播放头 0、媒体 0、draft null、html/body/viewport `1280/1280/1280`。Vite 产物 `index-CgIQHnAU.js`；完整后端 `307 passed in 222.79s`，compileall 与 diff check 通过。
- 发布证据：`188a650f` 已推送 `main`。标准重启 API `20196` / Worker `48516`，创建时间 `2026-08-20 10:08:07`；health ok、8766 listener 等于 API PID、Alembic runtime/head `20260816_52`、draft null。API 日志只有正常项目/健康/草稿读取与 Uvicorn 启动，Worker 日志为空。
