...
裁切把手必须使用事务式指针会话。Pointer down 只聚焦并冻结原始条目，不得立即写历史、清空重做或标记草稿；移动期间可临时预览波纹结果，只有 pointer up 时最终值确实不同才新增一个撤销步骤并保存草稿。没有移动、移动后回到原位都保持零修改；pointer cancel 必须移除全部监听并恢复会话前条目。左右把手 slider 同时支持键盘精调：方向键按真实输出 fps 移动一帧，Shift+方向键移动一秒，始终满足最短 200ms 和素材边界，并更新 `aria-valuenow / aria-valuetext`。键盘每次有效步进形成一个可撤销操作，锁轨时仍由统一门禁阻断。
