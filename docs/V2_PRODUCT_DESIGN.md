...
同步动作区还必须提供按需的单侧邻帧候选扫描，减少用户反复点击 `±1帧`、等待采样并记忆上一组指标的机械操作。用户选择前镜或后镜后，界面只挂载当前素材把手内相对 A 合法的 `−2 / −1 / +1 / +2` 帧候选，不后台扫描整段素材；每项展示候选源时码、明暗、综合色彩、逐像素指标及相对 A 的三项有符号百分点。候选按固定时间顺序呈现，不排序、不推荐、不标记“最佳”，指标方向仍不得解释为衔接更好或更差。点击“设为单侧 B”只把所选 delta 放入既有本地试调，并清空另一侧相位以保持单侧语义；此后继续复用同步/顺序试播、A→B 连播、采用 B、一次撤销和自动复检。滚动或转场试调会关闭扫描并保持三类语义互斥。扫描及选择均不写草稿，不新增 API、TimelineItem、EditorDraftSession、迁移、FFmpeg 或旧版兼容分支。
