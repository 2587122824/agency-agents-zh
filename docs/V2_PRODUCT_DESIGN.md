...
扫描区必须把当前侧、当前已挂载范围内的合法候选逐项审核进度直接汇总为“已对照数 / 总数”“本次保留 A 数”和“待复看数”，并提供统一的下一项入口。默认范围按 `−2 / −1 / +1 / +2`，显式扩展后按 `−4 / −3 / −2 / −1 / +1 / +2 / +3 / +4` 的冻结时间顺序重新投影总数和下一项；已经存在的 exact-key 结果必须保留，新增项只作为未看候选加入。“下一个”优先寻找第一个没有 exact-key 结果的候选并显示“对照下一个未看候选”；未看项为零但仍有 `shortlisted` 时，寻找第一个短名单候选并显示“复看下一个待复看”；两类都为空才显示完成状态。入口不读取证据数值、不按内容排序、不推荐最佳，用户显式点击后才复用既有“设为 B 并等待证据后 A→B”状态机。当前仍有 B 试调、A/B 播放或待决结论时入口必须禁用，要求用户先保留 A、暂存待复看、采用 B 或清除试调；每张卡始终可单独再次对照。进度与结果使用同一页面会话缓存和失效规则，不新增自动采用、草稿写入、API、迁移或持久化。
