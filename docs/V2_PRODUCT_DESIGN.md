...
当前创作智能体已经接入真实模型，但现有输入只包含用户消息，无法理解依赖助手上一轮回复的指代；同时自然回复和严格字段提取共用狭窄合同，导致重复追问和建议能力不足。下一版已经确认将其定义为“创作制片人”：完整读取当前会话中持久化的用户与助手消息，可以直接回答和提出结构化创意选项，但建议不自动成为项目事实。
...
智能体体系由创作制片人、内容策划、分镜导演、质量审核智能体和剪辑助理组成；确定性生产编译器独立于智能体。角色边界、合同和交接条件已经完成设计，详见 [创作中心设计](./V2_CREATION_CENTER_DESIGN.md#9-智能体编制与合同)。当前代码只实现其中一部分，准确状态见 [实现状态](./V2_IMPLEMENTATION_STATUS.md)。
