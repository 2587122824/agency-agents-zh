...
扫描区必须把当前侧合法候选的逐项审核进度直接汇总为“已对照数 / 总数”“本次保留 A 数”和“待复看数”，并提供“对照下一个未看候选”。“下一个”只按冻结的 `−2 / −1 / +1 / +2` 时间顺序寻找第一个没有 exact-key 结果的候选，不读取证据数值、不按内容排序、不推荐最佳；用户显式点击后才复用既有“设为 B 并等待证据后 A→B”状态机。当前仍有 B 试调、A/B 播放或待决结论时入口必须禁用，要求用户先保留 A、暂存待复看、采用 B 或清除试调；全部看完后显示完成状态，但每张卡仍可单独再次对照。进度与结果使用同一页面会话缓存和失效规则，不新增自动采用、草稿写入、API、迁移或持久化。
