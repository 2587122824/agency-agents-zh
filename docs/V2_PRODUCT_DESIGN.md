...
界面减法不能削弱衔接质量。`time_jump / location_change / outfit_change` 仍各保持 3 项人工判断，不新增第 4 项或新的同级入口；原有 `jump-readable / location-readable / outfit-readable` 直接承担“意图清楚且切点节奏自然”的合并判断，并要求同步动作观察与 1× 完整上下文切点均完成。用户看到的任务数量不增长，但不能再只凭两张定格图放行时间跳转、地点切换或换装切点。
...
当某项人工判断明确要求 action 观察时，同步动作区的当前任务必须直接展示“同步播放动作”和“顺序试播切点”两步，不得先把用户导向邻帧候选，也不得把必做播放藏进“其他无损试调”。两步观看只形成当前边界证据，不自动给出通过结论；邻帧候选、转场、滚动和相位仍作为判断不通过后的按需修复工具。
