...
观察指纹包含双方 client item、Asset、源入出点、成片入出点、fit，以及只属于该边界的前镜出场和后镜入场转场。任何裁切、滑移、滚动切位、片段移动、重排、素材替换或成对转场变化都会使旧证据不再匹配。观察证据与人工结果一起自动保存，但只证明用户实际使用了对应观察工具，不自动判断衔接好坏。生成新版本时服务端必须重新计算指纹，并要求每个 passed check 同时具备正确观察模式、精确指纹和观察时间，再冻结为 `timeline-continuity-review.v2`。
