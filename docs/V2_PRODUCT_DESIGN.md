...
定格对比还提供“动作帧带”视图，按 Timeline 真实输出帧率并排展示前镜切前 3、2、1 帧和后镜切后 0、1、2 帧。每格必须读取对应条目的冻结源区间、保持静音暂停，并显示精确源时间码；靠近素材边界时只允许钳制到该条目的首个或最后一个可解码输出帧，不能越界读取。点击任一格通过统一 seek 定位主监看，用户随后可用滚动剪辑移动切点或用源窗口滑移更换动作阶段。帧带只帮助人工判断动作阶段、运动方向和节奏断点，锁轨时仍可使用，不写草稿、撤销栈或连续性勾选，也不声称系统已完成视频理解。
