...
局部交换不能压过更完整的确定性修复。如果倒序边界右镜后紧邻显式画面缺口，并且系统能同时证明：右镜存在合法源尾延长、缺失正式 Shot 只有一个未使用已批准视频、延长与正式 Shot 后仍有至少 200ms 缺口、且该缺口只有一个未使用非正式补充视频能够覆盖，则普通 Inspector 主操作应改为“查看一次完整修复方案”。摘要前置列出延长量、正式分镜、补充素材覆盖量和目标总时长；点击只选择该缺口并展示精确方案，不立即改片。此时“仅交换这两镜”降为次级，专业衔接检查不再与结构修复并列。
