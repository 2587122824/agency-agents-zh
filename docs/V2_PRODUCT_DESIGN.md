...
首期已实现只读“已观测决策传播图”：只有被 `AgentInputManifest.decision_ids` 精确冻结的已解决决策，才会沿 AgentRun、候选、需求/方案版本、分镜、快照、DAG、素材和时间线外键形成下游证据。未观测表示没有现存传播证据，不表示可以断言没有业务影响。该页面不执行失效、重做、重试或费用估算，完整边界见 [V2 决策影响证据图实现](./V2_DECISION_IMPACT_IMPLEMENTATION.md)。
