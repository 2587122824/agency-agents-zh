...
只有 A→B 两阶段都完整播放后，界面才出现明确的本次对照结论区，避免把未看完的试调误当成已确认结果。“保留 A 原相位”必须清除页面内全部试调并回到 A，不写草稿、不产生撤销；“采用 B 当前试调”只在正在查看 B 且画面轨未锁定时启用，并自动收集全部非零 delta：只有一侧变化时复用该侧既有源窗口滑移事务，两侧都变化时复用双方组合原子事务，始终只形成一次撤销、保持成片位置和时长、重置相关连续性检查并自动试听写入后的真实切点。继续调整任一侧或重新启动 A→B 时必须隐藏旧结论；手动切到 A 可保留结论但不得采用不可见的 B。锁轨下保留 A 仍可用，采用 B 明确禁用。该决策层不新增 TimelineItem、EditorDraftSession、API、迁移、FFmpeg 或旧版兼容分支。
