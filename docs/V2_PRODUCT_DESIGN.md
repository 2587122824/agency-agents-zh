...
观察指纹包含双方 client item、Asset、源入出点、成片入出点、fit，以及只属于该边界的前镜出场和后镜入场转场。任何裁切、滑移、滚动切位、片段移动、重排、素材替换或成对转场变化都会使旧证据不再匹配。观察证据与人工结果一起自动保存，但只证明用户实际使用了对应观察工具，不自动判断衔接好坏。生成新版本时服务端必须重新计算指纹，并要求每个 passed check 同时具备正确观察模式、精确指纹、观察时间与完整步骤清单，再冻结为 `timeline-continuity-review.v4`。

动作连续性不能只凭并排同步播放判断。`action` 模式必须在同一当前边界指纹下分别完整观看 A 原方案的“同步动作”和“1× 顺序切点”，前者用于比较动作阶段与方向，后者用于按最终成片时间尺度观看前镜到后镜的真实 cut/fade 与切点节奏；两项缺一时“通过”保持禁用。0.25×/0.5× 慢放仍可用于逐帧分析动作，但改变了节奏感，完整播完也不能登记顺序切点证据；只有 1× 自然播放完前、后两段才登记。A→B 连续对照、B 当前试调、媒体就绪、播放失败或中止同样不能替代。权威观察保存严格有序 `completed_steps=['synchronous_action','sequential_cut_realtime']`；画面和叠加模式分别保存 `['left_frame','right_frame']` 与 `['overlay']`。草稿升级为 `editor-draft-session.v5 / editor-local-draft.v8`，冻结结果升级为 `timeline-continuity-review.v4`。
