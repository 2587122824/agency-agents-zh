...
成品回改采用版本化闭环，不把“不满意”直接等同于生成失败。用户在素材审核或剪辑预览中必须明确选择 `storyboard / production / editing` 三类问题之一，并填写依据；系统据此创建不可变 `AssetRevisionRequest`，冻结来源素材、生产快照、方案版本、精确镜头以及当时可观测的下游节点。分类由用户决定，后端不扫描理由文本猜测问题类型。
...
制作计划允许重复计算，以便用户在修改显式选择后重新检查步骤和费用；但同一项目内完全相同的冻结快照合同只能保存一次。唯一性依据是 `production-snapshot.v2` 完整合同的 `contract_hash`，不使用影响分析 ID、方案名称、预计费用或创建顺序判断。影响分析返回候选快照合同哈希，页面若已存在匹配快照则显示已有制作方案编号并取消重复保存入口；后端仍以项目 ID 与合同哈希执行权威 `409 PRODUCTION_SNAPSHOT_DUPLICATE` 门禁。历史重复快照不自动删除或合并；未来重做、重试和重新开版必须使用独立显式命令，不能复制相同快照绕过确认边界。
...
生产影响分析的默认结果按结构化错误码聚合同类问题，并从 `shots.<shot_code>.<field>` 精确路径匹配已冻结镜头编号，显示问题类型、影响镜头数量和编号；逐条原始错误仍完整保留在技术详情。主参考图状态默认显示已选择数量，逐镜头状态放在可展开区域。聚合只改变展示，不合并或解除后端校验，也不从错误文案猜测镜头、字段或修复方式。
...
真实外部生产执行使用进程环境中的显式总开关授权。开启总开关只允许后续由用户显式提交的新执行请求访问已配置 Provider，不得自动重跑历史失败或阻断任务，不得在开启时发送探测性付费请求。Provider 密钥保存在操作系统用户环境并通过凭据白名单注入 API 与 Worker，禁止写入系统配置数据库、事件、日志或仓库文件。
...
新确认的分镜方案使用 `shot-plan.v3`。每个镜头必须提供可直接交给视觉工作流的画面生成描述、结构化镜头语言、脚本段引用、精确人脸/文字约束、新增信息和生产能力要求；避免内容仍可不设置，系统不补默认负面词。主参考图必须由分镜导演候选或用户显式选择，不能自动采用列表第一项。

分镜编辑器只允许从该镜头已声明且具有已验证图片附件的实体中选择主参考图，并显示缩略图。移除对应实体时同步清除该次修订中的主参考选择；这属于同一条用户编辑命令，不会寻找替代图片。开发环境不提供旧分镜合同的运行时兼容分支，必须创建并确认新的 v3 候选后才能进入严格视觉生产。

已确认方案的人物补充采用显式版本链：方案页可以上传并登记人物参考版本，但上传本身不改变任何镜头；用户点击“绑定人物到具体镜头”后，后端从当前活动 PlanVersion 复制一份不可直接确认的修订草稿。用户逐镜头选择人物、主参考图、人脸可见性、人脸主体和身份一致性要求，提交 Patch 后生成待审候选，再次确认才创建新 PlanVersion；旧活动制作快照同时转为历史并解绑，必须基于新方案重新准备制作。该路径不调用模型、不自动套用全部镜头、不猜测人物或参考图，并允许用户显式放弃未提交草稿。
...
创作制片人的完整会话、结构化诊断与动态引导、结构化建议、点击与自然语言精确选择、选择后连续引导、待确认候选、新会话边界和失败审计已经实现。自然语言选择使用只读提案历史与当前 `reply_to` 选择作用域分离的最小权限合同，历史选择不能在无关轮次重复提交；结构化点击则由系统直接应用冻结值，模型只负责下一轮引导。动态引导结合当前草稿和会话选择一个最值得讨论的创作维度，不使用主题关键词特判或固定问卷顺序；创作限制只在用户明确提出时登记为可选事实，不由智能体主动补齐。建议范围仍限于内容方向、创作目标、观看感受、受众和风格，不替内容策划或分镜导演设计镜头、剪辑与生产参数。内容策划也已按独立 `planner` 模型分工完成：它只读取已确认需求、决策、实体和交付约束，生成内容承诺、开场、叙事节拍和脚本段候选；方案页按普通用户可读结构展示并保留人工接受门禁。智能体体系其余部分为分镜导演、质量审核智能体和剪辑助理；确定性生产编译器独立于智能体。角色边界、合同和交接条件详见 [创作中心设计](./V2_CREATION_CENTER_DESIGN.md#9-智能体编制与合同)，逐项完成度见 [实现状态](./V2_IMPLEMENTATION_STATUS.md)。
...
视频槽位分为首帧生视频与纯文本视频。首帧生视频必须同时明确选择一个图片槽位，并只消费该镜头唯一关键帧父节点；纯文本视频不得选择关键帧，不创建图片节点和父图片依赖。每个生产节点按 `plan_id + node_key` 生成并冻结确定性 seed，保证同一合同可审计重算；这不参与路由选择。可空避免内容在 NodeInfoList 中必须声明 `required=false`，缺值时不提交该绑定。

当前配置 v39 提供 6 个可执行镜头工作流槽位：通用参考图关键帧 `2072296894507872257`、纯文本关键帧 `2069402773254397953`、人物一致性关键帧 `2073414172825706497`、风格参考关键帧 `2066342754241835010`、首帧生视频 `2069607607387639810`、纯文本 B-roll 视频 `2071227330307125249`。每个槽位都有严格语义 NodeInfoList、支持规格和测试价格规则。

V1 中缺少工作流 ID 或 NodeInfoList 的多人物、首尾帧、多帧、真人转动漫、动作迁移、口型、增强和修复槽位不迁入；`i2v_first_middle_last_frame` 的 ID 与图片工作流冲突，`empty_transition_video` 的映射含提示词改写与固定业务文本，也不迁入。人物+场景双参考需要独立多参考图片合同，基础资产图和转面图属于实体生产阶段，均不得伪装成当前镜头生产能力。
