...
- 推荐 B 被排除后的唯一续办必须基于当前权威素材事实动态决定。有一个或多个未用于主画面的已批准视频时，主任务改为“为后镜选择替代素材”，进入现有素材箱替换模式并只列这些候选；没有候选时只显示“调整镜头结构”，退出衔接态。现有生产精确重试只允许重跑明确失败且请求合同未变的 WorkItem，不能为成功、已批准的镜头生成新变体；因此剪辑台不得伪造“重新生成”入口。确需新镜头时必须回生产流程创建新的版本化生产合同并重新确认费用。
