...
相邻主画面在 Inspector 中提供“镜头衔接”操作。切点预览从前镜结束前 1 秒播放到后镜开始后 1 秒，跨条目时继续沿用逐帧媒体时钟并在结束点自动暂停；画面轨锁定时仍允许该只读预览。衔接选择只提供直接切换和 0.2/0.3/0.5 秒柔和过渡：柔和过渡在一次 `commitItems` 中同时写前镜 `transition_out=fade` 与后镜 `transition_in=fade`，因此一次撤销必须整体恢复。这里复用已有 `cut/fade` 与 FFmpeg 合同，不伪装为会改变时间线时长语义的交叉叠化。
