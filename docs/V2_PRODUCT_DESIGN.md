...
本地兜底草稿使用显式 `editor-local-draft.v4`，必须与服务端草稿一样保存取整后的 `playhead_ms`。恢复与自动保存共用同一规范化语义指纹：只纳入基线 Timeline ID/行版本、稳定 TimelineItem 合同字段、播放头、磁吸和缩放；对象键递归排序，并忽略只为 UI 补充的 Asset 元数据。基线匹配且本地、服务端语义相同时必须优先服务端，把该指纹直接登记为已保存，不得因本地 `saved_at` 较新或对象键顺序不同在首次打开时重复 PUT；只有本地语义确实更新时才允许自动保存一次。因本地 schema 明确升级，旧版本地草稿直接失效，不增加运行时兼容分支。
...
三态结果仍是当前页面会话状态，不写入 EditorDraftSession、不可变 Timeline、history、API 或 localStorage。结构编辑继续清除受影响稳定边界的结果；正式连续关系变化时，只统计新关系清单中仍存在的检查 ID，旧关系结论不能抬高通过数。定位待处理切点继续走零播放头写入的只读 focus，只展开并排末帧/首帧、暂停所有媒体，不创建 B 或启动播放。
...
处理上下文仍是当前页面会话状态，不写 Timeline、EditorDraftSession、history、API 或 localStorage。项目/草稿重新载入和显式丢弃会清空；当前 main boundary 集合变化时，已不存在的 boundary key 必须同步裁剪，避免删除、重排后旧问题在未来相同页面回流。关系变化后当前检查清单已不含该 check ID 时不渲染上下文。重新复检只打开原工具、暂停媒体并保持播放头，不自动播放、不自动修改、不自动给出结论。
