...
同步动作模式允许分别对前镜或后镜做 `±1帧 / ±1秒` 的本地相位试调。试调只平移该侧比较媒体的有效源窗口并重新计算共同播放窗口，严格钳制在素材真实源把手内，不修改时间线草稿；界面必须同时显示试调后的精确源时间码和有符号前移/后移量，并可一键清除双方试调。只要任一侧已有试调，用户就可在“A 原相位”和“B 当前试调”之间反复切换与同步播放；切回 A 只把比较媒体临时恢复到基础源窗口，不能清除或改写已保存的本地 delta，再回 B 必须精确恢复试调源窗口。界面应明确标识当前正在观察 A 或 B，只有正在查看 B 时才允许应用，避免把不可见的保留试调误写入草稿。用户确认某一侧后，“应用相位”必须复用既有源窗口滑移事务：成片位置、片段时长与总时长不变，继续执行素材边界和画面轨锁定门禁，只产生一次撤销，重置目标片段相邻连续性检查，并在条目投影完成后自动顺序试听当前切点。双方都产生合法试调时，还可把两侧源窗口作为一个组合一次应用；该操作必须逐侧重新钳制素材把手，在同一次事务中写入双方且只形成一个撤销步骤，同时重置两侧片段全部相邻连续性检查并自动顺序试听当前边界。单侧应用继续保留。锁轨时仍可本地进行 A/B 比较，但所有应用按钮必须禁用并解释原因。该闭环不新增 TimelineItem、EditorDraftSession、迁移、FFmpeg 或旧版兼容分支。
