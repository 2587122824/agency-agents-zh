...
- “需调整”后的修复界面必须由检查类型决定唯一主路径。`sequence` 问题不扫描邻帧、不加载像素或动作幅度证据，也不显示同步动作与源窗口相位；当 A 为直接切换时，主任务只给出一个本地 `0.2s` 淡出淡入 B，当 A 已为淡出淡入时则只给出直接切换 B，随后立即进入真实顺序 A→B 对照。B 在明确“采用”前不写草稿；其他合法转场与滚动切位只放在“其他节奏方案”折叠区。
