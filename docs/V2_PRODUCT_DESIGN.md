...
相邻且两侧都有完整源区间的画面边界提供滚动剪辑。用户可把切点前移或后移 1 帧，也可按 1 秒粗调；操作在同一个撤销步骤中同时调整前镜 `source_out_ms / timeline_out_ms` 与后镜 `source_in_ms / timeline_in_ms`，因此总成片时长和所有后续片段结束时点保持不变。可移动范围由两侧素材真实边界、200ms 最短片段和已有淡入淡出不超过片段一半的合同共同限制，到达边界时控件禁用。每次切点变化都停止播放、定位新切点并重置该边界的临时人工检查进度；画面轨锁定时禁止修改，但切点预览和定格对比仍可用。
