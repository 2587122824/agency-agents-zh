...
切点静态两帧接近仍可能因两侧局部画面变化幅度不一致而产生顿挫，因此 A/B 主证据区还必须并列“切点动作幅度”。每套方案分别读取前镜末帧及其前一个可用输出帧、后镜首帧及其后一个可用输出帧，以同一逐像素平均绝对差算法显示“前镜末段变化 / 后镜开端变化 / 后镜减前镜”的一位小数百分比与有符号百分点；不足两个合法帧时明确显示哪一侧不可用。B 必须消费当前源窗口相位或滚动切位 delta，继续调整后重新寻帧；仅转场 B 与 A 使用同一组连续帧并说明只改变时间呈现。该指标只描述局部画面变化量，不推断动作速度或方向、主体身份、运动语义及衔接优劣。为控制解码器数量，它只挂载在 A/B 主证据区，不扩展到每张邻帧候选卡；结果仍只属于页面会话，不写草稿、Timeline、QC、正式复核或 FFmpeg 合同。
