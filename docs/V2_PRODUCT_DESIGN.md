...
持续引导不是后台循环或固定问卷。每次用户发送消息或点击选项只产生一次可审计 AgentRun；模型结合当前草稿和完整会话选择下一个焦点，后端只验证它没有重复刚选字段、没有重新登记已保存选择且没有越过创作制片人职责。非收口阶段必须同时给出非空 `focus_field` 与 `focus_reason`；达到 `ready_to_confirm` 时两者必须同时为 JSON `null`，助手总结并停止强制给选项，输入框仍开放。空字符串不会被系统补写或转换。模型失败时选择事实和草稿保留，用户确认费用后精确重跑失败 Run，不自动重试、切换模型或修复输出。
...
“当前配置”是用户工作区概念，不是可变数据库记录。发布修订后，新版本成为当前配置；上一版本自动进入历史入口。页面可以让用户感觉是在维护同一份配置，但后端仍必须创建不可变版本，不能覆盖已经被快照、工作项或智能体运行引用的记录。生成服务连接准备只展示当前生产配置的供应商状态，历史版本的连接事实仅在历史审计中读取。
...
当前发布配置 v25 保持关键帧工作流 `2072296894507872257`、Provider、模型、视频工作流和价格不变，仅将 `shot.negative_prompt` 与 `reference_image.primary` 两个关键帧绑定声明为可选，`reference_image.present` 仍为必填布尔开关。这使实际配置与 `shot-plan.v2` 的可空字段合同一致，不属于无参考图降级、默认负面词补写或工作流替换。
...
创作制片人的完整会话、结构化诊断与动态引导、结构化建议、点击与自然语言精确选择、选择后连续引导、待确认候选、新会话边界和失败审计已经实现。自然语言选择使用只读提案历史与当前 `reply_to` 选择作用域分离的最小权限合同，历史选择不能在无关轮次重复提交；结构化点击则由系统直接应用冻结值，模型只负责下一轮引导。动态引导结合当前草稿和会话选择一个最值得讨论的创作维度，不使用主题关键词特判或固定问卷顺序；建议范围仍限于内容方向、创作目标、观看感受、受众和风格，不替内容策划或分镜导演设计镜头、剪辑与生产参数。内容策划也已按独立 `planner` 模型分工完成：它只读取已确认需求、决策、实体和交付约束，生成内容承诺、开场、叙事节拍和脚本段候选；方案页按普通用户可读结构展示并保留人工接受门禁。智能体体系其余部分为分镜导演、质量审核智能体和剪辑助理；确定性生产编译器独立于智能体。角色边界、合同和交接条件详见 [创作中心设计](./V2_CREATION_CENTER_DESIGN.md#9-智能体编制与合同)，逐项完成度见 [实现状态](./V2_IMPLEMENTATION_STATUS.md)。
...
模型只调用一次，失败不创建候选。系统不补字段、不修复 JSON、不改写 ID、不自动重试、不切换模型或生产路由。用户可在方案页确认再次调用模型后，精确重跑当前需求版本最近一次失败运行；重跑复用原输入清单和完全一致的配置合同，并保留原失败证据。成功输出仍是待审核候选，结构化人工修订也必须重新通过相同合同，最终由用户确认后才创建不可变方案版本。

方案页的结构化人工修订通过单镜头编辑器完成：左侧镜头列表用于定位、搜索和筛选，右侧只展示当前镜头字段；移动端上下排列。切换镜头不提交数据，最终仍按原始 `shot_code` 汇总发生变化的镜头 Patch 并创建下一版候选，不增加镜头 ID 猜测、自动保存或输出修复。
