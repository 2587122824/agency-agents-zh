...
扩展范围会使候选卡及其动作证据在窄 Inspector 中形成长列表，因此进度区下方必须提供固定时间顺序的候选快速导航。每个导航项只显示帧偏移和当前页面态：`未看 / 已实测 / 当前 B / 待决定 / 保留 A / 待复看`；状态继续从当前 delta、exact-key 动作记忆和人工结果同步派生，不能保存第二份可漂移状态。点击导航项只把对应候选卡聚焦并滚动到当前 Inspector 可见区域，不选择候选、不创建 B、不启动证据或媒体、不改变审核结果、不排序或推荐。候选卡必须有唯一稳定的边界、侧别、帧偏移 DOM 标识和可编程聚焦目标；导航只属于本页可访问性与浏览效率，不写草稿、history、API、localStorage 或迁移。
