...
持续引导不是后台循环或固定问卷。每次用户发送消息或点击选项只产生一次可审计 AgentRun；模型结合当前草稿和完整会话选择下一个焦点，后端只验证它没有重复刚选字段、没有重新登记已保存选择且没有越过创作制片人职责。达到 `ready_to_confirm` 时助手总结并停止强制给选项，输入框仍开放。模型失败时选择事实和草稿保留，用户确认费用后精确重跑失败 Run，不自动重试、切换模型或修复输出。
