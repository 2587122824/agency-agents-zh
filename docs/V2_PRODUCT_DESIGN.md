...
候选列表必须进一步区分“已得到实测数据”“用户已完整看完 A→B”和“用户本次明确保留 A”。只有某个单侧 `±1/±2` 帧候选的 A、B 两阶段都完整顺序播放后，才在该 exact-key 候选卡登记“已完整对照 A→B / 尚未选择”；仅设为 B、只看单阶段、播放中止、证据失效或媒体未就绪均不得登记。用户在当前结论区点击“保留 A”后，同一 exact key 更新为“本次已选择保留 A”；再次完整对照该候选时先恢复为尚未选择，防止旧结果冒充新一轮决定。切换其他候选后结果保留，便于逐项核对；采用 B 后基线改变，条目、素材、基础源窗、fps 或边界变化时与实测动作记忆一起清空。结果只表示本页人工操作历史，不代表系统优劣、推荐或正式复核，不写草稿、Timeline、history、API 或浏览器持久化。

扫描区必须把当前侧合法候选的逐项审核进度直接汇总为“已对照数 / 总数”和“本次保留 A 数”，并提供“对照下一个未看候选”。“下一个”只按冻结的 `−2 / −1 / +1 / +2` 时间顺序寻找第一个没有 exact-key 结果的候选，不读取证据数值、不按内容排序、不推荐最佳；用户显式点击后才复用既有“设为 B 并等待证据后 A→B”状态机。当前仍有 B 试调、A/B 播放或待决结论时入口必须禁用，要求用户先保留 A、采用 B 或清除试调；全部看完后显示完成状态，但每张卡仍可单独再次对照。进度与结果使用同一页面会话缓存和失效规则，不新增自动采用、草稿写入、API、迁移或持久化。
