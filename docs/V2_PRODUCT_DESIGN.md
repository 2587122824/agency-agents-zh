...
相邻且两侧都有完整源区间的画面边界提供滚动剪辑。用户可通过按钮或 `, / .` 快捷键把当前切点前移或后移 1 帧，按住 `Shift` 时粗调 1 秒；操作在同一个撤销步骤中同时调整前镜 `source_out_ms / timeline_out_ms` 与后镜 `source_in_ms / timeline_in_ms`，因此总成片时长和所有后续片段结束时点保持不变。可移动范围由两侧素材真实边界、200ms 最短片段和已有淡入淡出不超过片段一半的合同共同限制，到达边界时控件禁用。每次成功变化都基于更新后的条目自动播放当前窗口、当前速度的单次切点预览，不自动循环且不增加第二个历史步骤；按钮、快捷键和动作帧带必须复用同一门禁和自动试听入口。快捷键只作用于当前聚焦或选择推断出的相邻边界，在文本输入、原生表单、业务弹窗或 `Alt/Ctrl/Meta` 组合键期间不得穿透；画面轨锁定时禁止修改且不得制造撤销历史，但切点预览和定格对比仍可用。
