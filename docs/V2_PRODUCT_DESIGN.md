...
候选列表必须进一步区分“已得到实测数据”和“用户已完整看完 A→B”。只有某个单侧 `±1/±2` 帧候选的 A、B 两阶段都完整顺序播放后，才在该 exact-key 候选卡登记“已完整对照 A→B”；仅设为 B、只看单阶段、播放中止、证据失效或媒体未就绪均不得登记。切换其他候选后标记保留，便于逐项核对；条目、素材、基础源窗、fps 或边界变化时与实测动作记忆一起清空。标记只表示本页观看进度，不代表优劣、推荐、结论或已采用，不写草稿、Timeline、history、API 或浏览器持久化。
