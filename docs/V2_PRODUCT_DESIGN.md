...
用户明确结束阻断生产后，旧快照进入 `superseded`，只保留历史审计，不再占用当前制作方案。此时重新分析并保存完全相同的冻结合同必须创建新的 `snapshot_id` 与递增制作方案编号，重新经过费用锁定、激活和提交确认；不得复活旧快照、复用旧 WorkItem、自动提交或自动重试。相同 `contract_hash` 跨越两个已明确分隔的生产轮次是合法历史，不表示数据库重复。
