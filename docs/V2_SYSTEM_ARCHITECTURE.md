...
- 同步动作相位试调由 `BoundaryActionComparison` 内部的 `leftPhaseDeltaMs / rightPhaseDeltaMs` 保存。每侧 delta 的下限为 `-source_in_ms`，上限为 `asset_duration_ms - source_out_ms`；有效源入出点均由基础源区间加 delta 得到，RAF 终点和共同窗口继续只消费这组有效范围。试调、清除和两路媒体定位不进入 `items` 指纹、草稿自动保存或撤销历史。单侧应用回调把合法 delta 交给既有 `slipBoundaryItem`；条目更新后同步媒体随 `items` 变化清理，并由 `pendingBoundaryPreviewKey` 在新投影上启动真实顺序试听。该链路不新增 API、迁移、FFmpeg 合同或旧版兼容逻辑。
