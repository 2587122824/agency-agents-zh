...
- `editorDraftRestoreComplete` 是草稿写入总门禁：项目重置为 false，恢复 effect 完整注入 items、连续性和候选 session 后才为 true。localStorage 与 API autosave 在 false 时均不得写入，防止 React 恢复批次的中间态覆盖权威候选审核。
