...
- 动作问题驱动的候选引导保持在前端页面会话边界。父页面只在当前 outcome 精确为 `needs_adjustment` 且检查模式为 `action` 时发出一次带 `boundaryKey / checkId / checkLabel / requestToken` 的瞬时请求；动作比较组件在既有切点/试调清理 effect 之后消费请求，按双方当前源窗分别投影默认 `−2/−1/+1/+2` 合法候选，优先左侧非空、否则右侧非空，并立即消费父请求，避免组件重挂载恢复旧引导。组件本地保留问题标签用于当前会话说明；并排、叠加、普通观察、候选记忆续办和 outcome 已重置的 recheck 不创建请求，项目切换与结构状态重置会清空未消费请求。当前侧耗尽且另一侧默认候选非空时才派生继续按钮。该投影不读取动作数值、不改变候选顺序、不创建 B、不播放、不提交条目，也不新增草稿字段、API、schema、迁移、history、Timeline 或 FFmpeg 分支。
