...
- `editorDraftRestored` 显式门禁候选 session 的结构失效清理。在远端/本地草稿择优并注入 items 与 session 之前，初始空 `mainBoundaries` 不允许删除候选审核；恢复完成后才按当前稳定 session key 过滤。该门禁防止 React effect 的旧空边界闭包覆盖刚恢复的数据。
- 草稿冲突选择把无 `Z`/offset 的服务端 SQLite `updated_at` 明确解释为 UTC，再与 localStorage 的 ISO UTC 时间比较；不能让浏览器按本地时区误读服务端时间，从而错误选择旧本地草稿覆盖权威候选审核。
