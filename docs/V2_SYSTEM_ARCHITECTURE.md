...
- `BoundaryMotionProbe` 只在 A/B 主证据区为每套可见方案挂载四路 1px 离屏静音媒体：前镜 `boundary-frameStepMs / boundary` 与后镜 `boundary / boundary+frameStepMs`。四个 `seeked` 目标都通过 20ms 时钟核对后，下一 RAF 分别复用 `analyzeBoundaryPixels(...).pixel_change_percent` 计算两段逐像素变化，并投影 `round((right-left)*10)/10` 的有符号百分点。连续帧目标按各自有效源入点和 `source_out-frameStepMs` 钳制；不足一帧间距时直接显示不可用。A 使用基础时点，B 同时消费左右相位与滚动 delta；转场-only 自然读取与 A 相同的四帧。候选列表不挂载该 probe，避免每项再增加两路解码器。全部 readiness、结果与错误只属于组件生命周期，不进入 `items` 指纹、EditorDraftSession、history、Timeline、QC、API、迁移或 FFmpeg。
