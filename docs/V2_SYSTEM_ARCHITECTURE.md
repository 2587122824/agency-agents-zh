...
- `BoundaryMotionProbe` 只在 A/B 主证据区为每套可见方案挂载四路 1px 离屏静音媒体：前镜 `boundary-frameStepMs / boundary` 与后镜 `boundary / boundary+frameStepMs`。四个 `seeked` 目标都通过 20ms 时钟核对后，下一 RAF 用 `readFramePixels` 把每帧缩放到固定 48×48；`analyzeFrameMotion` 一次遍历同时得到全画面逐像素平均绝对差，以及固定 3×3、每格 16×16 的区域变化百分比。组件投影总量 `round((right-left)*10)/10`，并逐格投影后镜减前镜有符号百分点；区域底色的 alpha 只按变化百分比连续映射，不携带好坏阈值。连续帧目标按各自有效源入点和 `source_out-frameStepMs` 钳制；不足一帧间距时直接显示不可用。A 使用基础时点，B 同时消费左右相位与滚动 delta；转场-only 自然读取与 A 相同的四帧。候选列表不挂载该 probe，避免每项再增加两路解码器。全部 readiness、结果与错误只属于组件生命周期，不进入 `items` 指纹、EditorDraftSession、history、Timeline、QC、API、迁移或 FFmpeg。
