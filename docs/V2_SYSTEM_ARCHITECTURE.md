...
- `activeBoundaryCompleteRepair` 是只读投影，不新增第二套执行事务。它只在当前倒序边界右侧紧邻 gap 时，按既有缺口规则重新证明安全延长量、唯一正式 Shot 素材、剩余缺口与唯一非正式 filler；依赖项与 `selectedGap*` 使用同一当前 mainItems、Shot 映射、已用素材集合和活动 workspace。命中后主入口只 `selectItem(gap)` 并把 Inspector 滚到顶部，实际应用仍由原 `applySelectedGapCombinedRepair(true)` 再次消费选中 gap 的完整派生与锁轨门禁。局部交换回退继续复用 `swapBoundaryToFormalOrder`。
...
- 变化型正式边界复用既有检查 ID：`jump-readable / location-readable / outfit-readable` 映射到 `sequence`，只要求 `completed_steps == [sequential_cut_realtime_context]`；`same_moment.motion` 继续映射 `action`，要求 `[synchronous_action, sequential_cut_realtime_context]`。两种模式都要求 `playback_rate == 1`、左右上下文覆盖各自最多 1000ms 且边界 fingerprint 精确匹配。合同升级为 `editor-draft-session.v10 / editor-local-draft.v13 / timeline-continuity-review.v7`，迁移 `20260816_52` 重建开发连续性进度；旧 Timeline 不改写，也不增加运行时兼容分支。
...
- `BoundaryActionComparison` 的引导意图显式区分 `observe / issue / resume`。`observe` 只设置当前问题和观察模式，不打开候选、不选择扫描侧；`sequence` 一级只展示顺序试播，`action` 才展示同步播放与顺序试播。`issue` 在用户明确“需调整”后打开修复候选，`resume` 恢复既有候选待办。媒体按钮只改变现有会话和 ready evidence，最终 outcome 仍需用户明确选择。

- 衔接 Inspector 的修复工具可见性由权威页面投影计算，而不是新增持久化 UI 状态：只有当前 boundary 的 `EditorContinuityIssueContext` 仍对应 `needs_adjustment`，或该 boundary 存在候选审核续办，`repairToolsVisible` 才为真。该门禁统一控制普通切点预览参数、滚动、源窗口滑移、模式切换与外层转场编辑；观察按钮仍按 mode 精确挂载 frames/overlay/action/sequence 内容。全时间线连续巡检按钮也只在修复态或正在运行时保留。可见性不进入 local draft、EditorDraftSession、history、TimelineItem 或后端合同，不需要迁移。
...
- `BoundaryActionComparison` 以瞬时 `guidedIntent + guidedReviewMode` 选择修复状态机。`issue/sequence` 直接构造与当前成对转场相反的 `sequencePrimaryTrial`：cut 基线试用最多 `200ms` fade，paired fade 基线试用 cut；`comparisonEvidenceReady` 在 sequence 下只要求存在 B，顺序媒体 ready 与完整 A→B 播放仍由既有媒体状态机门禁，不再等待 `BoundaryPixelProbe / BoundaryMotionProbe`。action 分支保持原 source-key 像素与动作证据门禁。
- sequence 分支不挂载动作证据、邻帧扫描或双方相位区；其顺序舞台只在播放时显示，“其他节奏方案”独立控制转场和滚动试调。页面从当前 `EditorContinuityIssueContext.mode` 派生 `directBoundaryToolsVisible`：仅 `frames / overlay` 控制外层 preview/roll/slip/transition/frame modes，`action / sequence` 只挂载内部比较组件。上述状态均不进入 local draft、EditorDraftSession、history、TimelineItem 或后端合同，继续使用 v10/v13/v7 与迁移 52。
- sequence 引导请求显式携带原 `checkId`，组件只在页面态保存当前引导 ID。纯函数推荐表将 `jump-readable / location-readable / outfit-readable` 分别映射为 `200 / 300 / 500ms` 成对 fade；最大时长仍由双方片段各半时长与 2000ms 上限裁剪。只有基线两侧均为 fade、时长相等且精确命中当前语义推荐时，主 B 才改为 cut；其他已有 fade 仍对照语义目标时长。该推荐不读取画面指标、模型输出或历史候选，不改变试调、A→B、一次采用与草稿边界。
- sequence 首选试调复用 `candidate_review_sessions[exact_session].alternative_outcomes`，键为 `sequence-primary:<checkId>:<type>:<durationMs>`，值继续只允许既有 `kept_baseline`。`chooseTransitionTrial` 把该键放入既有 `alternativeReviewTrialKey`，完成 A→B 后的 `keepBaselinePhase` 走同一追加合并与自动保存路径；无需升级 v10/v13 草稿、Pydantic、迁移或 Timeline v7。当前 exact session 读到该键时不再创建同一 B，主卡调用既有 `onAdjustStructure`，父级同时选择前镜、关闭衔接 Inspector 和临时比较状态，回到片段态；不产生 history、播放或剪辑写入。
