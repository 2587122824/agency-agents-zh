...
- `BoundaryMotionProbe` 只在 A/B 主证据区为每套可见方案挂载四路 1px 离屏静音媒体：前镜 `boundary-frameStepMs / boundary` 与后镜 `boundary / boundary+frameStepMs`。四个 `seeked` 目标都通过 20ms 时钟核对后，下一 RAF 用 `readFramePixels` 把每帧缩放到固定 48×48；`analyzeFrameMotion` 一次遍历同时得到全画面逐像素平均绝对差、固定 3×3 每格 16×16 的区域变化百分比，以及以像素差为权重的 X/Y 坐标累积。总差大于零时再对 2304 个已缓存差值计算到重心的加权平均距离，并以画面对角线归一为分散度；总差为零时重心显式为 `null`。组件投影总量、逐格后镜减前镜、重心坐标/分散度及两侧差值，网格定位点只消费归一化坐标；区域底色的 alpha 只按变化百分比连续映射，不携带好坏阈值。A/B probe 通过 `onAnalysis` 回传同一已计算对象，父级以四个源时点拼成 source key 拒绝旧结果，再由 `boundaryMotionDeltas` 逐项计算 B−A 的三项总量、两套九格和两侧重心差；该摘要不挂载新媒体。连续帧目标按各自有效源入点和 `source_out-frameStepMs` 钳制；不足一帧间距时直接显示不可用。A 使用基础时点，B 同时消费左右相位与滚动 delta；转场-only 自然读取与 A 相同的四帧，因此差值确定性为零。候选列表不挂载该 probe，避免每项再增加两路解码器。全部 readiness、结果与错误只属于组件生命周期，不进入 `items` 指纹、EditorDraftSession、history、Timeline、QC、API、迁移或 FFmpeg。
