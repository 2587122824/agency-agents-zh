...
`shot-plan.v2` 不新增项目状态或自动转移。候选生成后仍必须经过用户审核，只有确认成功才创建新的不可变 PlanVersion。缺少画面生成描述、实体引用无效、主参考不属于镜头或参考附件不可验证时，候选保持未接受并返回逐字段错误，不创建方案版本。

生产影响分析和快照创建具有第二道确定性守卫：历史 `shot-plan.v1`、工作流输入映射不完整、必填输入为 `null`、附件越界或文件 MIME/大小/哈希不一致时返回结构化阻断。此时不创建可执行 WorkItem，不调用 Provider，也不改变项目为“已生产”。

生产提交与 RunningHub 上传前再次校验快照冻结事实。校验失败只阻断对应真实工作项及其 required 下游；不会改写分镜、重新选择参考图、去掉必填映射、切换工作流或提交第二次尝试。可选的避免内容或主参考图为 `null` 时，仅省略相应可选 NodeInfoList 条目，这是合同定义，不是降级路径。

本合同沿用现有项目状态转移、事件信封、Outbox 和人工确认机制，不新增自动恢复、自动重试或费用事件。历史 v1 方案需要用户显式创建并确认 v2 修订，不能由迁移或 Worker 触发状态推进。
