...
| 结构化分镜编辑 | 已完成 | `shot-plan.v2` 类型化逐镜头 patch、候选替代链、行版本冲突、画面生成描述、可空避免内容、人物/服装/场景/产品与显式主参考图选择；方案页采用镜头导航与单镜头编辑器，支持搜索、已修改筛选、修改计数、前后切换和单镜头重置。桌面左右布局、移动端上下布局，工作区固定高度；桌面镜头列表与表单分别独立滚动，列表不会撑高工作区或被底部栏截断。切换不自动保存，仍一次提交原有 Patch，不开放自由 JSON |
