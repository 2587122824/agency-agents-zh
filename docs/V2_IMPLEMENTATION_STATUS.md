...
| 创作制片人动态引导合同 | 已完成 | `creative-dialogue-input.v5 / output.v5 / prompt.v13`、结构化项目类型与创作阶段判断、已明确字段/关键缺口、本轮唯一焦点、聚焦理由和消息证据均持久化；首次进入和每次点击选择后都会由用户动作授权一次模型调用，基于累积草稿继续引导下一个高价值缺口。后端验证字段互斥、来源、焦点与建议一致性，并禁止选择后重复登记或再次询问刚选字段；诊断不修改正式需求、不决定系统就绪、不选择模板或生产路由，不使用主题关键词特判或固定问卷顺序。失败保留选择且只允许用户确认费用后精确重跑；成功重跑会解除对应失败链的当前阻断并清理页面临时错误，旧失败仍保留在审计历史中；不自动重试 |
