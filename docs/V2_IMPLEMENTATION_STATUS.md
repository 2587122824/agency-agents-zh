...
| 工作流槽位注册表 | 已完成 | 版本化系统配置、WorkflowSlot、NodeInfoList 验证和设置页；配置及组件技术键由界面生成并隐藏，关联项按名称选择，支持的视频规格按列表勾选。设置页采用单一当前配置工作区：主列表只展示最新发布版，未发布修改单独提示，历史版本折叠保留；编辑会复用现有草稿或显式创建下一版草稿，连接准备只展示当前配置。配置 v25 保持原关键帧工作流不变，仅把合同本就可空的避免内容与主参考图绑定改为 `required=false`，参考图存在开关仍显式提交 |
