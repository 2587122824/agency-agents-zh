...
| 工作流槽位注册表 | 已完成 | 版本化系统配置、WorkflowSlot、NodeInfoList 验证和设置页；当前配置 v35 保持三个文本智能体使用 `deepseek-v4-flash`，不虚构 DeepSeek 图片能力。原 RunningHub 工作流、NodeInfoList、视频规格、音频、存储与价格保持不变。关键帧参考图能力来自显式 NodeInfoList；首帧视频仍只声明 `i2v / first_frame`，没有虚构多帧或精确文字标签 |
...
| RunningHub Provider | 部分完成 | 图片和首帧视频 Adapter、严格 NodeInfoList、`production-work-request.v3`、单父图片、显式主参考附件上传、提交任务号持久化、重启轮询恢复与确定性本地下载已用假传输验证；历史 v1 不回填，文件篡改在联网前阻断。用户已于 2026-07-20 明确授权真实外部生产，Windows 用户环境已配置密钥、凭据白名单与执行开关；开启动作本身不重跑旧任务、不提交付费请求，真实 RunningHub 端到端生成仍待用户新建或显式重跑任务验收 |
