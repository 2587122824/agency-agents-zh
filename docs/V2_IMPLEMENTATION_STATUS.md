...
| 结构化分镜编辑 | 已完成 | `shot-plan.v4` 类型化逐镜头 Patch、覆盖视图、结构化枚举、脚本段/人物/主参考图/人脸主体/能力要求复选、首中尾画面描述、精确文字和新增信息编辑均已接入。正式方案可显式开启不可直接确认的修订草稿；草稿可放弃，提交并再次确认才创建新 PlanVersion。人工修订不调用模型、不自动套用全部镜头；不自动保存、不开放自由 JSON |
