...
| Repository 接口 | 部分完成 | 已完成 Project/Event/Decision/Command/Creation/Planning/Production/Quality/Editor/Delivery/Work/Configuration 协议、SQLAlchemy 实现和合同测试；核心命令服务、Worker 与配置服务已无直接 ORM 查询，其余只读投影仍待迁移 |
...
| 决策影响图 | 部分完成 | 已有生产配置/快照影响分析；尚无通用 Decision 到实体、镜头、素材的影响图 |
...
1. 继续按聚合迁移 Repository，并维持 API 和事务语义不变。
2. 补结构化分镜候选修订合同；若候选版本语义需要改变，实施前确认。
3. 设计通用决策影响图，只输出影响证据，不自动失效、重做或重试。
4. 单独评审项目状态转移器与 Outbox；二者都不得在重构中顺带引入。
5. Provider、OSS、FFmpeg 与重试保持冻结，直到用户明确授权对应边界。
