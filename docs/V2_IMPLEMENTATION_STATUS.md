...
| 能力 | 状态 | 权威证据与缺口 |
|---|---|---|
| 对话、需求版本、方案版本 | 已完成 | 用户/助手 `Message`、不可变需求草稿修订链、`RequirementVersion`、`PlanVersion` 与创作/规划 API；首次进入按主题执行一次持久化引导，连续消息和点击选择继承当前草稿，模型失败保留上一修订；只有最终确认才创建正式需求并进入策划。失败不自动重试或换模型；对话和需求面板桌面等高并内部滚动，输入区固定在底部 |
