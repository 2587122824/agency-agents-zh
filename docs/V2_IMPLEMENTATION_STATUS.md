...
| 创作制片人动态引导合同 | 已完成 | `creative-dialogue-input.v5 / output.v5 / prompt.v12`、结构化项目类型与创作阶段判断、已明确字段/关键缺口、本轮唯一焦点、聚焦理由和消息证据均持久化；后端验证字段互斥、来源、焦点与建议一致性，页面紧凑展示诊断依据。首次引导、累积草稿、精确选择、单字段建议和失败边界保持不变；诊断不修改需求、不决定系统就绪、不选择模板或生产路由，不使用主题关键词特判或固定问卷顺序；配置 v19 与真实 DeepSeek 首次引导已验收 |
