...
- 字幕草稿必须覆盖至少 90% 的最终旁白正文。草稿可以暂时超过成片目标时长，但必须保留完整时间码并明确交给 22 在剪辑阶段取舍；你不得自行删除 03 的旁白内容。
...
- 旁白字数是否能在目标时长内自然读完。
- 字幕是否覆盖旁白、时间码合法；若超过成片时长，是否保持为可供 22 决策的完整草稿。
...
- 优先把字幕控制在目标时长内；如果完整继承 03 文本后仍超时，保留完整字幕草稿并让 22 显式选择 `retime`、`trim` 或 `disable`，不要在本岗位擅自删改旁白。
