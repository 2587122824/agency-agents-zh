...
- 可在 `全自动生成` 中选择只出制作包、出制作包 + 配音字幕文本、只生图不生视频，或 `全自动成片预览：调用 ComfyUI 素材接口 + FFmpeg 自动剪辑`；当前会自动整理 `production_manifest.json`、生图提示词、视频提示词、语音字幕包、ComfyUI 参数包、字幕、配音文本和剪辑清单，供后续工具继续执行。
...
只生图，不生视频：只调用已接入的独立生图适配器；不会调用生视频适配器。
全自动成片预览：调用 ComfyUI 素材接口 + FFmpeg 自动剪辑，先下载云端 ComfyUI 生成的图片/视频素材，再用本地 FFmpeg 合成 `final_video.mp4` 预览版。
```

当前版本会生成可执行资产包和 manifest；选择 `只生图，不生视频` 时只会调用已接入的生图适配器，不会调用生视频适配器。选择 `全自动成片预览`，并填写密钥、接口地址和 ComfyUI 工作流接口时，会调用 ComfyUI 素材适配器；自动剪辑方式选择 `本地 FFmpeg 自动剪辑预览` 时，会继续尝试输出 `final_video.mp4`。
