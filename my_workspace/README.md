...
6. 由 `20_语音字幕包装师` 生成 TTS 配音稿、字幕、BGM、音效和混音建议。
7. 由 `21_ComfyUI素材编排师` 把 AI 图片、AI 视频、语音字幕和自动化要求整理成 ComfyUI / RunningHub 素材参数。
8. 由 `22_剪辑成片执行师` 把脚本、素材片段、配音字幕和封面整理成剪辑工具可执行的最终成片方案。
8. 游戏类工作流由 `08` 到 `14` 号员工协作，从项目编排、GDD、Unity 架构、关卡叙事、3D 美术技术、音频体验，到 Steam 发行测试制作包。
9. 软件市场机会分析工作流由 `15_软件市场需求分析师` 输出高潜力软件方向、MVP、获客渠道、商业化和风险验证。
10. AI 员工工作流平台设计由 `15` 到 `19` 号员工协作，输出平台定位、员工管理、工作流架构、技术架构和增长验证方案。
...
生成视频 + 语音字幕制作包：增加 TTS、字幕、BGM、音效和混音参数。
...
- `20_语音字幕包装师`：基于脚本和视频片段方案生成 TTS 配音稿、字幕、BGM、音效和混音建议。
- `21_ComfyUI素材编排师`：把生图、生视频、语音字幕和自动化要求整理成 ComfyUI / RunningHub 素材节点参数、执行模式和回退方案。
- `22_剪辑成片执行师`：把脚本、AI 素材片段、配音、字幕和封面整理成剪辑工具可执行的最终成片方案。
...
具体生图/生视频提示词由 `02_短视频编导` 先根据原始需求设计内容和镜头方向，再由 `06_分镜生图设计师` 拆成关键帧方案，由 `07_视频生成执行员` 整理成视频生成包，并供 `21_ComfyUI素材编排师` 整理节点映射。AI 图片和 AI 视频只作为素材片段，最终成片由 `22_剪辑成片执行师` 交给剪辑工具完成。
