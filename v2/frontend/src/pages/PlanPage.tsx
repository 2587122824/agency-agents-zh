...
function ShotTable({ shots, locked }: { shots: ShotContract[]; locked: boolean }) {
  return <div className={styles.shotTableWrap}><div className={styles.tableTitle}><div><Clapperboard size={17} /><h3>分镜合同</h3></div><span>{shots.length} 个镜头 · {(shots.reduce((sum, shot) => sum + shot.duration_ms, 0) / 1000).toFixed(1)} 秒</span></div><div className={styles.tableScroll}><table><thead><tr><th>镜头</th><th>画面生成描述</th><th>实体与主参考</th><th>约束</th><th>时长</th><th>状态</th></tr></thead><tbody>{shots.map(shot => <tr key={shot.shot_code}><td><strong>{shot.shot_code}</strong><small>{shot.shot_type}</small></td><td><strong>{shot.visual_prompt ?? '缺少画面生成描述'}</strong><small>{shot.negative_prompt ? `避免：${shot.negative_prompt}` : '避免内容未设置'} · {shot.composition}</small></td><td><span>{shot.character_entity_version_ids.length ? shot.character_entity_version_ids.join(', ') : '人物未绑定'}</span><small>{shot.primary_reference_entity_version_id ? `主参考 ${shot.primary_reference_entity_version_id}` : '无主参考图'}</small></td><td><span>人脸 {shot.face_visibility}</span><small>文字 {shot.text_policy} · 动态 {shot.motion_requirement}</small></td><td>{(shot.duration_ms / 1000).toFixed(1)}s</td><td><em data-locked={locked}>{locked ? '已锁定' : '待确认'}</em></td></tr>)}</tbody></table></div></div>
}
...
export function PlanPage() {
  ...
  const retryBrief = useMutation({ mutationFn: () => api.retryCreativeBrief(projectId, planning.data!.latest_planner_run!.id, planning.data!.active_requirement.id), onSuccess: refresh })
  ...
  const error = generateBrief.error || retryBrief.error || decideBrief.error || generateShots.error || reviseShots.error || decideShots.error || preparation.error || analyzeImpact.error || createSnapshot.error || lockSnapshot.error || activateSnapshot.error || submitProduction.error
