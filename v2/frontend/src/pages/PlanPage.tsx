...
export function PlanPage() {
  ...
  const error = generateBrief.error || decideBrief.error || generateShots.error || decideShots.error || preparation.error || analyzeImpact.error || createSnapshot.error
  ...
  function chooseConfig(value: string) { setConfigId(value); setVideoSpecId(''); setKeyframeSlotId(''); setVideoSlotId(''); setTtsSlotId(''); analyzeImpact.reset() }
  return <>
      ...
      <section className={styles.main}>
        ...
        {data.active_plan && <section className={styles.productionPrep}>
          ...
          {impact && <div className={styles.impactResult} data-blocked={impact.status === 'blocked'}>
            <header><Calculator size={17} /><div><strong>{impact.status === 'blocked' ? '生产合同存在阻断' : '影响分析等待确认'}</strong><span>{impact.manifest.shots.length} 个镜头 · {impact.estimated_call_count} 次计划调用 · 不会立即执行</span></div><code>{impact.analysis_hash.slice(0, 12)}</code></header>
