...
const readinessLabels = { connected: '执行能力已就绪', adapter_not_connected: '执行组件尚未接通', configuration_not_ready: '生成配置需要更新', execution_disabled: '真实执行授权未开启', credential_not_ready: 'API Key 尚未填写' } as const
const apiKeyLabels = { not_required: '不需要 API Key', missing: 'API Key 未填写', configured: 'API Key 已填写' } as const
const nextActionLabels = { connect_adapter: '先接入对应的执行组件', revise_configuration: '复制此版本并更新工作流输入', configure_credential: '编辑当前配置并填写 API Key', enable_execution: '确认真实测试前再开启执行授权', ready: '可以用于新生产任务' } as const
...
function ReadinessChecks({ provider }: { provider: ProviderReadinessItem }) {
  const checks = [
    ...
    { label: 'API Key', ready: !provider.credential_required || provider.api_key_state === 'configured' },
...
export function SettingsPage() {
  ...
  return <><PageHeader eyebrow="SYSTEM AUTHORITY" title="系统配置" description="日常只维护当前生产配置；历史版本由系统保留，用于项目追溯。" actions={<><button className="secondaryButton" onClick={() => client.invalidateQueries()}><RefreshCw size={14} />刷新</button>{currentVersion ? <button className="primaryButton" disabled={busy} onClick={() => prepareEdit.mutate()}><Settings2 size={15} />{activeDraft ? '继续编辑' : '编辑配置'}</button> : <button className="primaryButton" onClick={beginCreate}><Plus size={15} />创建生产配置</button>}</>} />
    <section className={styles.connectionPanel}>
      <header><div>{readiness.data?.external_execution_enabled ? <PlugZap /> : <Unplug />}<span><strong>生成服务连接准备</strong><small>按执行组件、生成配置、API Key 和执行授权逐项检查；不会联网或产生费用。</small></span></div><em data-ready={readiness.data?.external_execution_enabled}>{readiness.data?.external_execution_enabled ? '可以执行' : '尚未准备完成'}</em></header>
      <div className={styles.connectionList}>
        {currentReadiness.map(provider => <article key={`${provider.configuration_version_id}:${provider.provider_version_id}`} data-status={provider.status}>
          ...
          <details><summary>技术详情</summary><code>{provider.adapter_kind} · {provider.capabilities.join(', ') || 'NO_CAPABILITY'} · api_key={apiKeyLabels[provider.api_key_state]} · contract_issues={provider.configuration_issue_codes.join(', ') || 'none'}</code></details>
        </article>)}
    ...
    <div className={styles.page}>
      ...
      <main className={styles.workspace}>{editing ? <form className={styles.editor} onSubmit={submit}>
        <div className={styles.editorHead}><div><span>{selected.data ? `REVISE v${selected.data.version_number}` : 'NEW DRAFT'}</span><h2>{selected.data ? '修订配置草稿' : '创建配置草稿'}</h2><p>技术标识由系统生成并在修订中保持不变。API Key 与供应商配置一起保存，修改后发布新版本生效。</p></div><button type="button" className="iconButton" title="关闭编辑" onClick={() => setEditing(false)}><X size={16} /></button></div>
        ...
        <section className={styles.formSection}><header><Server /><div><strong>服务供应商</strong><span>API 地址、API Key 与能力声明统一维护</span></div><button type="button" className="secondaryButton" onClick={() => setDraft({ ...draft, providers: [...draft.providers, providerDraft()] })}><Plus size={13} />供应商</button></header>{draft.providers.map((item, index) => <div className={styles.repeatBlock} key={item.provider_key}><div className={styles.repeatTitle}><b>服务供应商 {index + 1}</b>{draft.providers.length > 1 && <button type="button" className="iconButton" title="删除供应商" onClick={() => removeList('providers', index)}><Trash2 size={14} /></button>}</div><div className={styles.formGrid}><label>显示名称<input required value={item.display_name} onChange={event => updateList('providers', index, { ...item, display_name: event.target.value })} /></label><label>适配器类型<input required value={item.adapter_kind} onChange={event => updateList('providers', index, { ...item, adapter_kind: event.target.value })} placeholder="runninghub / openai_compatible / local" /></label><label>API 地址<input required type="url" value={item.base_url} onChange={event => updateList('providers', index, { ...item, base_url: event.target.value })} /></label><label>API Key<input value={item.api_key ?? ''} onChange={event => updateList('providers', index, { ...item, api_key: event.target.value || null })} placeholder="填写该供应商提供的 API Key" autoComplete="off" spellCheck={false} /></label><label>区域<input value={item.region ?? ''} onChange={event => updateList('providers', index, { ...item, region: event.target.value || null })} /></label><label className={styles.wide}>能力（逗号分隔）<input required value={item.capabilities.join(', ')} onChange={event => updateList('providers', index, { ...item, capabilities: event.target.value.split(',').map(value => value.trim()).filter(Boolean) })} /></label><label>请求超时（秒）<input required type="number" min="1" value={item.request_timeout_seconds} onChange={event => updateList('providers', index, { ...item, request_timeout_seconds: Number(event.target.value) })} /></label><label>轮询间隔（秒）<input required type="number" min="1" value={item.poll_interval_seconds} onChange={event => updateList('providers', index, { ...item, poll_interval_seconds: Number(event.target.value) })} /></label><label>最大并发<input required type="number" min="1" value={item.max_concurrency} onChange={event => updateList('providers', index, { ...item, max_concurrency: Number(event.target.value) })} /></label></div></div>)}</section>
        ...
        <section className={styles.formSection}>
          ...
          {draft.workflow_slots.map((item, index) => <div className={styles.repeatBlock} key={item.slot_key}>
            ...
            <div className={styles.formGrid}>
              ...
              <label>生成类型<select required value={item.operation_kind} onChange={event => updateList('workflow_slots', index, { ...item, operation_kind: event.target.value })}><option value="">请选择</option><option value="image_generation">生成图片</option><option value="video_generation">首帧生成视频</option><option value="multi_frame_video_generation">首中尾帧生成视频</option><option value="text_to_video_generation">文字生成视频</option><option value="tts">文字生成语音</option>{!['', 'image_generation', 'video_generation', 'multi_frame_video_generation', 'text_to_video_generation', 'tts'].includes(item.operation_kind) && <option value={item.operation_kind}>其他：{item.operation_kind}</option>}</select></label>
