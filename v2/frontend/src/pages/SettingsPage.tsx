  ...
  return <><PageHeader eyebrow="SYSTEM AUTHORITY" title="系统配置" description="配置先验证、再发布；项目快照只引用已发布的精确版本。" actions={<><button className="secondaryButton" onClick={() => client.invalidateQueries()}><RefreshCw size={14} />刷新</button><button className="primaryButton" onClick={beginCreate}><Plus size={15} />新建配置草稿</button></>} />
    ...
    <div className={styles.page}>
      ...
      <main className={styles.workspace}>{editing ? <form className={styles.editor} onSubmit={submit}>
        ...
        <section className={styles.formSection}><header><KeyRound /><div><strong>模型注册表</strong><span>可选；AgentRun 后续按精确版本引用</span></div><button type="button" className="secondaryButton" onClick={() => setDraft({ ...draft, models: [...draft.models, modelDraft()] })}><Plus size={13} />模型</button></header>{draft.models.length === 0 && <div className={styles.inlineEmpty}>本配置暂未登记模型，不会自动选择任何模型。</div>}{draft.models.map((item, index) => <div className={styles.repeatBlock} key={item.config_key}><div className={styles.repeatTitle}><b>模型 {index + 1}</b><button type="button" className="iconButton" title="删除模型" onClick={() => removeList('models', index)}><Trash2 size={14} /></button></div><div className={styles.formGrid}><label>显示名称<input required value={item.display_name} onChange={event => updateList('models', index, { ...item, display_name: event.target.value })} /></label><label>Agent 角色<select value={item.agent_role} onChange={event => updateList('models', index, { ...item, agent_role: event.target.value as ModelConfigDraft['agent_role'] })}><option value="creative">Creative</option><option value="director">Director</option><option value="qc">QC</option><option value="editor">Editor</option></select></label><label>所属服务供应商<select required value={item.provider_key} onChange={event => updateList('models', index, { ...item, provider_key: event.target.value })}><option value="">请选择</option>{draft.providers.map((provider, providerIndex) => <option key={provider.provider_key} value={provider.provider_key}>{provider.display_name || `服务供应商 ${providerIndex + 1}`}</option>)}</select></label><label>供应商模型 ID<input required value={item.provider_model_id} onChange={event => updateList('models', index, { ...item, provider_model_id: event.target.value })} /></label><label>输入合同版本<input required value={item.input_contract_version} onChange={event => updateList('models', index, { ...item, input_contract_version: event.target.value })} /></label><label>输出 Schema 版本<input required value={item.output_schema_version} onChange={event => updateList('models', index, { ...item, output_schema_version: event.target.value })} /></label><label>Prompt 合同版本<input required value={item.prompt_contract_version} onChange={event => updateList('models', index, { ...item, prompt_contract_version: event.target.value })} /></label></div></div>)}</section>
