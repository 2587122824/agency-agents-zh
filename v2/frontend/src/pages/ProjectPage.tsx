    ...
    <main className={styles.layout}>
      <section className={styles.conversation}>
        ...
        <div className={styles.messages} ref={messagesRef}>
          ...
          {(initializeConversation.isPending || generate.isPending || selectSuggestion.isPending || retryCreativeTurn.isPending) && <article data-role="assistant" data-pending><span>AI</span><div><p>{retryCreativeTurn.isPending ? '正在按你的确认重跑本轮…' : selectSuggestion.isPending ? '选择已提交，正在继续为你梳理下一步…' : initializeConversation.isPending ? '正在根据主题准备创作方向…' : '正在理解本轮需求…'}</p><small>本次只运行一次当前配置的创作模型</small></div></article>}
          {(initializeConversation.error || generate.error || selectSuggestion.error || retryCreativeTurn.error) && <article data-role="system" data-error><span>系统</span><div><p>{selectSuggestion.error ? '选择可能已保存，但后续引导没有完成：' : '本轮智能体没有返回回复：'}{(initializeConversation.error || generate.error || selectSuggestion.error || retryCreativeTurn.error)?.message}</p><small>没有自动重试，也没有切换模型</small></div></article>}
        </div>
        <form className={styles.composer} onSubmit={submit}>
          ...
          <div><button type="button" className="secondaryButton" onClick={() => fileInput.current?.click()} disabled={!creationOpen || initializeConversation.isPending || selectSuggestion.isPending || register.isPending}><Paperclip size={15} />{register.isPending ? '上传中…' : '上传附件'}</button><input ref={fileInput} hidden type="file" accept="image/png,image/jpeg,image/webp,audio/wav,audio/mpeg,video/mp4" onChange={event => { const file = event.target.files?.[0]; if (file) register.mutate(file); event.target.value = '' }} /><span>{creationOpen ? '选择后会调用一次创作模型并继续引导' : '当前需求版本已进入策划阶段'}</span><button className="primaryButton" disabled={!creationOpen || initializeConversation.isPending || !message.trim() || addMessage.isPending || generate.isPending || selectSuggestion.isPending || retryCreativeTurn.isPending}>发送<Send size={15} /></button></div>
        </form>
      </section>
      ...
      <section className={styles.requirements}>
        ...
        {creation.next_action.code === 'RETRY_FAILED_CREATIVE_TURN' && creation.latest_agent_run && <div className={styles.retryBox}><div><CircleAlert size={18} /><p><strong>{candidate ? '选择已保存，后续引导失败' : '本轮创作智能体运行失败'}</strong><span>{creation.latest_agent_run.error_detail || '模型没有返回有效结果。'} 系统不会自动重试。</span></p></div><button className="primaryButton" onClick={() => retryCreativeTurn.mutate()} disabled={retryCreativeTurn.isPending}>{retryCreativeTurn.isPending ? '正在重跑…' : '确认模型调用并重跑本轮'}</button></div>}
