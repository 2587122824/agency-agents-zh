...
class EditorDraftSession(Base):
    ...
    schema_version: Mapped[str] = mapped_column(String(40), default="editor-draft-session.v11")
    track_config: Mapped[dict] = mapped_column(JSON)
    items: Mapped[list] = mapped_column(JSON)
    playhead_ms: Mapped[int] = mapped_column(Integer, default=0)
    continuity_outcomes: Mapped[dict] = mapped_column(JSON, default=dict)
    continuity_issue_contexts: Mapped[dict] = mapped_column(JSON, default=dict)
    continuity_observations: Mapped[dict] = mapped_column(JSON, default=dict)
    candidate_review_sessions: Mapped[dict] = mapped_column(JSON, default=dict)
    row_version: Mapped[int] = mapped_column(Integer, default=1)
...
class AudioConfigVersion(Base):
    ...
    voice_presets: Mapped[list] = mapped_column(JSON, default=list)
    default_voice_key: Mapped[str | None] = mapped_column(String(80), nullable=True)
    ...
    speaking_rate_default: Mapped[float] = mapped_column(default=1.0)
    volume_range: Mapped[dict] = mapped_column(JSON, default=lambda: {"min": 0, "max": 100})
    volume_default: Mapped[int] = mapped_column(Integer, default=50)
    duration_tolerance_ms: Mapped[int] = mapped_column(Integer, default=1500)
